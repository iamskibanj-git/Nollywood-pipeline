...
class ThumbnailGenerator {
  ...
  async generateThumbnail(params) {
    ...
    if (sceneImagePath && fs.existsSync(sceneImagePath)) {
      ...
      await this.generateKeyArt({
        characterNames, setting, mood, genre, outputPath: keyArtPath,
      });
    }
    ...
    await this.generateTitleCard({
      title, tagline, preset, outputPath: titleCardPath,
    });
   ...
   */
  async generateKeyArt({ characterNames, setting, mood, genre, outputPath }) {
    ...
    const prompt = `Cinematic 16:9 key art for a Nollywood ${genre} drama. ${charRefs}
...
- NO text, NO title, NO watermarks — key art only`;
    ...
    console.log(`[THUMBNAIL] Stage 1: Generating key art...`);
    try {
   ...
   */
  async generateTitleCard({ title, tagline, preset, outputPath }) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      console.log(`[THUMBNAIL] Stage 2: Generating title card (attempt ${attempt}/${maxAttempts})...`);
      try {
        await this.automation.generateImage({
          prompt,
          outputPath,
