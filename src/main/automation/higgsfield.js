...
class HiggsFieldAutomation {
  ...
  async recoverTimedOutImage(submittedPrompt, outputPath, opts = {}) {
    ...
    for (let i = 0; i < tilesToCheck.length; i++) {
      ...
      try {
        ...
        console.log(`[IMG RECOVERY] Tile ${i + 1} similarity: ${similarity}% (need ≥${minSimilarity}%)`);
        ...
        console.log(`[IMG RECOVERY] ✓ MATCH at ${similarity}% — downloading ${tile.uuid}...`);
        ...
        try {
          ...
          if (stat.size < 10000) {
            console.warn(`[IMG RECOVERY] Download too small (${stat.size} bytes) — likely a placeholder`);
          ...
          console.warn(`[IMG RECOVERY] Download failed for tile ${i + 1}: ${dlErr.message.split('\n')[0]}`);
          continue;
        }
      } catch (tileErr) {
        console.warn(`[IMG RECOVERY] Error checking tile ${i + 1}: ${tileErr.message.split('\n')[0]}`);
        continue;
      }
    }
  ...
  /**
   * Normalized word-overlap similarity for prompt comparison.
   * Same algorithm as cinema-studio-automation and kling-automation.
   */
...
module.exports = { HiggsFieldAutomation };
