...
2026-07-13 YouTube Community/non-video post implementation note: Engagement reuse for non-video YouTube posts now targets YouTube Community posts separately from Shorts and Facebook. The working UI route is Studio dashboard -> Content -> Posts for management/proof, while creation/scheduling must use the public channel Community composer at `https://www.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA/community?show_create_dialog=1`. Do not use the older `my_community?pdc=...` route; it can show "Posts aren't currently available on this device." The Community composer uses the inline `What's on your mind?` prompt, then the split `Post` button's `Action menu`, then `Schedule post`; date/time are custom controls `#date-picker` and `#time-picker`, with date values like `Jul 18, 2026` and 15-minute time options like `12:15 AM`. Scheduled Community posts do not appear on the public Community page before publication; proof must be read back from Studio `Content > Posts`, where the row shows `Text`, `Scheduled`, the scheduled date, and an edit URL.

2026-07-13 YouTube Community scheduled-only E2E proof: a text-only scheduled post was created for `fayebaba` and must not be duplicated. Caption starts "Scheduled E2E test for the Nollywood pipeline YouTube Community automation." It is scheduled for `Jul 18, 2026` at `12:15 AM` local time and appears in Studio `Content > Posts` with remote post id `Ugkxc-3JVyZGxP7W-Eya324d2JV_Pck1C1mU` / edit URL `https://studio.youtube.com/post/Ugkxc-3JVyZGxP7W-Eya324d2JV_Pck1C1mU/edit`. User said scheduled-post E2E was required and can be canceled later; do not retry the live scheduler unless this specific test row has first been canceled/deleted or a new explicit test is requested.

2026-07-13 YouTube Community E2E cleanup: user manually deleted the scheduled E2E Community post `Ugkxc-3JVyZGxP7W-Eya324d2JV_Pck1C1mU` after proof was captured. The live test row is no longer expected to exist in YouTube Studio.
2026-07-13 YouTube Community post-merge smoke and image fix: copied-DB Electron Engagement smoke on `master` verified `Prepare YouTube` and `Schedule YouTube` button wiring for SCWOB target date `2026-07-31`; prepare created 3 Community jobs, with 1 ready portrait post and 2 blocked oversized scene-image posts (>16 MB). Live scheduled-only smoke first exposed a media proof bug: setting an existing hidden file input reported success but Studio scheduled the row as `Text`. The mismatched text-only remote post `UgkxQqv6g20CpxW4OmHgYN-WIUFpXq2mzynG` was deleted from Studio. Code was hardened to click the visible `Image` control, set files through the filechooser/latest image input, require visible attachment proof (`Cancel image post` / `Edit preview` / image preview), and scroll offscreen composer controls into view before clicking. Final live proof scheduled SCWOB social post `257` as an `Image` Community post for `Jul 31, 2026` at `12:00 PM`; Studio `Content > Posts` readback showed `Image`, `Scheduled`, `Jul 31, 2026`, remote post id `Ugkx3Lurn_pTBwaH_HzlpvpGetYWLU0VAIYy`, edit URL `https://studio.youtube.com/post/Ugkx3Lurn_pTBwaH_HzlpvpGetYWLU0VAIYy/edit`.
2026-07-13 YouTube Community oversized media follow-up: `Prepare YouTube` now auto-exports oversized Community images to cached JPG derivatives under `assets/social/youtube-community/` using ffmpeg, preserves already scheduled jobs instead of resetting them to ready, and reports preserved jobs separately from ready/blocked counts. Live July 31 SCWOB batch proof: `ch10_sc03_cinematic.png` (41080315 bytes) was exported to `assets/social/youtube-community/ch10_sc03_cinematic_yt_community.jpg` (512687 bytes). Existing noon post `257` stayed scheduled as `Ugkx3Lurn_pTBwaH_HzlpvpGetYWLU0VAIYy`. Remaining July 31 posts were scheduled successfully as image posts: post `258` at `15:00`, remote id `UgkxDgOOUvJBr5dpNAuf2TJi4uBO_ZiRGZxf`, edit URL `https://studio.youtube.com/post/UgkxDgOOUvJBr5dpNAuf2TJi4uBO_ZiRGZxf/edit`; post `259` at `21:00`, remote id `UgkxYTuqy8OiLAAV9kjEfOrfLY9TzS-mT72Q`, edit URL `https://studio.youtube.com/post/UgkxYTuqy8OiLAAV9kjEfOrfLY9TzS-mT72Q/edit`. Studio `Content > Posts` readback showed `Image`, `Scheduled`, and `Jul 31, 2026` for both.
2026-07-13 YouTube Community companion-gate update: YouTube Community posts are no longer treated as an independent Community calendar. They are platform publish jobs over the existing Facebook-style engagement bundle around a scheduled Short: 12 PM character/spotlight, 3 PM teaser, 6 PM YouTube Short, and 9 PM recap. `Prepare YT Companions` must resolve the matching `short_publish_jobs` row for the source `short_id` with `platform = 'youtube_shorts'`, `status = 'scheduled'`, same scheduled date, expected `18:00` Short time, and remote proof (`remote_post_id` or `remote_url`) before marking a Community job ready. Missing/mismatched Short proof blocks the Community job and scheduling re-checks the companion gate before opening YouTube. Companion metadata is persisted on `social_publish_jobs.metadata_json.companion`, including short publish job id, remote proof, schedule date/time, post type, and offset minutes. Already-scheduled Community jobs keep remote proof but prep may annotate companion validation if the anchor Short is missing.
2026-07-13 YouTube Community companion-gate smoke: prep-only smoke against the real AppData SQLite for SCWOB `2026-06-15_b904d6af`, target date `2026-07-31`, opened no browser and scheduled nothing. Result: 3 Community jobs prepared, 0 ready, 0 newly blocked, 3 preserved because remote Community proof already exists (`257/258/259`), but `companionSummary.blocked = 3` because source short `115` has no matching `short_publish_jobs` row for `platform = 'youtube_shorts'` on `2026-07-31` at `18:00`. The three scheduled Community jobs now carry companion error `Matching scheduled YouTube Short job not found for short 115`; rerun `Prepare YT Companions` after the 6 PM Short is scheduled to attach companion proof and clear the stale companion error.

2026-07-13 YouTube Shorts duration guard update: YouTube's current Shorts limit is 3 minutes for square-or-taller videos, so the old initial-live-proof warning/block at 60 seconds was removed from src/main/shorts/platform-profiles.js. Keep the hard validation gate at maxDurationSeconds = 180 and vertical/square aspect; 75s SCWOB Shorts should prepare/schedule normally, while 181s+ remains blocked.

2026-07-13 YouTube Shorts live anchor proof: SCWOB short `115` / short #31 (`"It Was Always Meant To Come Home"`, 75.375s, 1080x1920) was prepared as `short_publish_jobs.id = 5` and live scheduled in YouTube Studio for `2026-07-31 18:00`. Studio `Content > Shorts` readback confirmed title match, `Scheduled`, `Jul 31, 2026`, remote video id `DfiOAAF7FcQ`, edit URL `https://studio.youtube.com/video/DfiOAAF7FcQ/edit`. The YouTube Short uploader needed a date-picker hardening: the old day button click missed `31`, and a too-broad direct date input selector accidentally filled the time input with `7/31/2026`; `src/main/shorts/youtube-studio-uploader.js` now avoids generic date-input matching and falls back to a visible date-picker day-cell DOM click before final Schedule. After the Short proof existed, `Prepare YT Companions` against SCWOB target date `2026-07-31` preserved the three already scheduled Community image posts and attached companion proof to all of them: prepared 3, ready 0, blocked 0, preserved 3, `companionSummary.ready = 3`, `companionSummary.blocked = 0`, offsets -360/-180/+180 minutes around the 6 PM Short.

2026-07-13 YouTube Community lead-order hardening: hardcode the rule that YouTube Community must never lead the engagement bundle. Backend companion validation no longer honors `requireYouTubeShortCompanion = false`; status exposes `youtubeShortCompanionSummary`; and the Social tab disables/blocks `Prepare YT Companions` plus `Schedule YT Companions` until the selected engagement rows have matching 18:00 YouTube Short remote proof. Facebook remote proof is intentionally not part of this YouTube Community gate; the required upstream proof is the scheduled YouTube Short.
...
Update 2026-06-29 standalone "how to" Facebook content research module analysis:
- User wants a new standalone module unrelated to the Nollywood production pipeline, but placed in this repo to reuse existing Playwright/Higgsfield/Facebook automation. High-level spec: weekly Node.js pipeline for eight Facebook "how to" pages; scrape Reddit, Google Trends, Pinterest, YouTube autocomplete, and Quora; score/dedupe/rewrite with Claude `claude-sonnet-4-6`; write `raw_topics.json`, `scored_topics.json`, and `posts_queue.json`; later feed image generation and Facebook scheduling.
- Best reusable Facebook asset is `src/main/social/social-facebook-uploader.js`. It already schedules Facebook image posts through the Professional Dashboard Content Library: open Create Post, upload image, verify preview, enter caption with read-back, open scheduling modal, set date/time, click Schedule post, and confirm via scheduled-row polling/refresh. It extends `src/main/shorts/facebook-uploader.js`, which provides persistent Playwright profile login, popup dismissal, schedule-window checks, date/time needle matching, and scheduled row discovery.
- Best reusable Higgsfield asset is `src/main/automation/higgsfield.js` via `HiggsFieldAutomation.generateImage({ prompt, outputPath, aspectRatio })`. For this how-to module, call this directly with each scored `image_prompt` to make clean editorial images. Do not bend the Nollywood `ThumbnailGenerator`/promo card flow unless the user explicitly wants title-card compositing; that flow is character-element and drama-thumbnail specific.
- Best reusable orchestration pattern is `src/main/promo/index.js`: plan -> generate assets -> generate copy -> schedule. It also demonstrates using `SocialFacebookUploader` and Higgsfield in one feature. Reuse the pattern, not the Nollywood character logic.
- Best reusable Claude patterns are `src/main/social/social-copy-generator.js` and `src/main/promo/promo-copy-generator.js`: construct strict prompts, call Anthropic SDK, strip markdown fences, parse JSON, validate outputs, and retry once with rejection feedback. The attached scorer prompt is different and should live in a new scorer module.
- Do not reuse `src/main/social/social-planner.js` directly. It is tightly coupled to Nollywood projects, SQLite `social_posts`, scripts, shorts, character bible entries, portraits, scene images, and scheduled Reels. It is useful only as a reference for status summaries and scheduling ergonomics.
- Prefer a file-based standalone v1 matching the spec, not the existing SQLite `social_posts` table. Recommended structure: `pipeline/config.js`, `pipeline/scraper.js`, `pipeline/scorer.js`, `pipeline/queue.js`, `pipeline/run.js`, plus later `pipeline/adapters/higgsfield-image-generator.js` and `pipeline/adapters/facebook-scheduler.js`. Because the repo is currently CommonJS while the spec requests ESM, give `pipeline/package.json` its own `"type": "module"` and use `createRequire()` only when wrapping existing CommonJS modules.
- Important gap before scheduling eight pages: current Facebook scheduler assumes the active logged-in Professional Dashboard/page context. It does not explicitly select among eight Facebook pages. Add config-driven page/account targeting or separate persistent browser profiles/page URLs before live scheduling.
- Keep v1 side-effect-light: scrape -> score -> queue only. Later v2 can add image generation and scheduling fields such as `page_id`, `page_name`, `scheduled_date`, `scheduled_time`, `image_path`, `caption`, and `status`. Use existing 29-day Facebook schedule-window deferral from `FacebookUploader` when scheduling.

Follow-up 2026-06-29 Facebook multi-page context switch proof:
- User-provided screenshots confirmed the practical page-switch route for the eight "how to" pages. Start from a logged-in personal Facebook session, click the top-right Account/profile icon, click `See all profiles`, use the `Your profiles & Pages` modal/panel, choose the exact target page such as `Fix It`, wait for the account context to switch, then open `Professional dashboard`, select `Content`, and use `Content Library`. At that point the existing image-post scheduling flow can continue.
- The proof signals in the screenshots: the modal lists pages (`Fix It`, `Look Good`, `Tech It`, `Make It`, `Get Fit`, `Grow It`, `Cook It`); selecting `Fix It` changes the left sidebar/composer to the page context (`Fix It`, `What's on your mind, Fix It?`); the left sidebar exposes `Professional dashboard`; after entering `/professional_dashboard/content/content_library/`, the page shows Professional dashboard -> Content -> Content Library with the target page avatar at top right.
- Implementation note: add a `targetPageName` or page config field before scheduling. In `SocialFacebookUploader` or a standalone adapter, run `ensureFacebookPageContext(targetPageName)` before `_reloadScheduledLibrary()` / `scheduleImagePost()`. If the active page already matches, skip switching.
- Recommended automation flow: click the top-right Account/profile button, not the app grid; click an exact visible target page if present, otherwise click `See all profiles`; in `Your profiles & Pages`, optionally type the target into the search field, then click the exact row match; wait for proof such as `Switched to <page>`, the composer placeholder containing the page name, left nav page name, or the account menu marking the target page active. Only after page-context proof should the code navigate to `https://www.facebook.com/professional_dashboard/content/content_library/`.
- Do not trust the dashboard URL alone as page proof because it is generic. Verify the target context after navigation using visible page name/avatar/menu state before clicking `Create`. This prevents posting a how-to item to the wrong page if Facebook silently stays in the personal profile or previous page context.

Implementation 2026-06-29 standalone how-to pipeline v1:
- Added `pipeline/` as a standalone ESM subpackage with `config.js`, `scraper.js`, `scorer.js`, `queue.js`, `run.js`, `logger.js`, `package.json`, `.env.example`, `.gitignore`, and `README.md`. It is intentionally file-based and isolated from the Electron/SQLite Nollywood app.
- Current safe flow is scrape -> score -> queue only. It writes `raw_topics.json`, `scored_topics.json`, `posts_queue.json`, and `logs/run_YYYY-MM-DD.log`. It does not call Higgsfield image generation or Facebook scheduling yet.
- `config.js` defines all eight niches and includes `facebook_page_name` for future scheduling/page-switch work. `run.js` supports `--skip-scrape`, `--skip-score`, `--skip-queue`, `--no-login-pause`, and `--headless`; it loads optional `pipeline/.env` without adding a dotenv dependency.
- `scraper.js` launches headed Chromium, optionally uses `CONTENT_RESEARCH_USER_DATA_DIR`, shows a manual login holding page, waits for Enter, and scrapes Reddit, Google Trends, Pinterest, YouTube autocomplete/results fallback, and Quora. Per-source failures are warnings and do not abort the niche.
- `scorer.js` uses Anthropic `claude-sonnet-4-6`, the attached how-to strategist prompt, 1500ms delay between niches, fence-stripping JSON parse, raw-response failure logs, and skips only the failed niche on parse/API errors.
- `queue.js` flattens scored topics into `posts_queue.json` rows with `niche_id`, `niche_name`, `facebook_page_name`, `rank`, `topic`, `hook`, `image_prompt`, `sources`, and `status: queued`.
- Validation run: `npm.cmd run check` passed after PowerShell blocked `npm.ps1`; direct ESM imports of config/logger/scraper/scorer/queue also passed and reported `niches=8`.

Follow-up 2026-06-29 standalone key handling:
- The attached spec was only a suggestion, not gospel. For this repo, the standalone pipeline should reuse existing app credentials when available. `pipeline/credentials.js` now resolves keys in this order: `ANTHROPIC_API_KEY`, `CLAUDE_API_KEY`, then the existing Electron config `%APPDATA%\nollywood-ai-pipeline\config.json` field `claudeApiKey`. Do not log or print key values.
- Keep env vars as the override for cron/alternate billing contexts, but the normal local workflow can use the same Claude key already saved in the Electron app settings.

Follow-up 2026-06-29 standalone Reddit source plan:
- The user-provided subreddit list is useful and has been promoted into `pipeline/config.js`: each of the eight niches has three primary subreddits and two backup subreddits. `r/DIY` intentionally appears under both Fix It and Make It because the Claude scoring prompt receives the niche/page context and can separate repair content from craft/build content.
- Reddit scraping now uses `/top/?t=month` rather than hot/current feed noise. It extracts post title, upvote count, and comment count when visible; comment count is preserved in `engagement_detail` and weighted as a secondary signal for sorting before scoring.
- Default Reddit quality gate is at least 100 upvotes for both primary and backup subs. Backup subs expand coverage only when primary subs return fewer than `redditMinItemsBeforeBackup` usable items; they should not lower the signal floor unless deliberately tuned later.
- Scraper skips pinned/mod-like posts via visible/title/attribute heuristics and preserves subreddit/group/min-upvote metadata for downstream scoring/debugging.

Follow-up 2026-06-29 standalone Google Trends source plan:
- The second source note is useful and now informs `pipeline/scraper.js`. Each niche uses two Trends seeds in `google_trends_keywords`, e.g. Fix It = `how to fix` + `home repair`, Cook It = `how to cook` + `easy recipe`, Grow It = `how to grow` + `gardening tips`, Money = `how to make money` + `side hustle`, Get Fit = `how to lose weight` + `home workout`, Make It = `how to make` + `DIY craft`, Tech It = `how to use AI` + `ChatGPT tips`, Look Good = `how to style` + `skincare routine`.
- Trends URL pattern is `https://trends.google.com/trends/explore?q={keyword}&geo=US&date=now+7-d`. Keep `geo`, `date`, per-seed limits, render timeout, and delay jitter configurable in `pipeline/config.js`.
- Scraper targets Related Queries -> Rising first, falls back to Top if fewer than `googleTrendsMinRisingBeforeTop` rising rows are found, and also pulls Related Topics -> Rising as a secondary cluster signal when available. It retains a broad page-text fallback because Google Trends selectors can drift.
- `Breakout` is scored as engagement `999`; percentage values become numeric engagement; Top fallback rows receive a low baseline score. Preserve `engagement_detail` with `seed`, `section`, `tab`, `value`, `geo`, and `date` so Claude can reason about why a topic is timely.
- Google Trends should fail soft per seed/source: handle cookie/consent popups, wait specifically for related-query-ish widgets before scraping, warn and skip/fallback after render timeout, and avoid aborting the whole weekly run. Use 2-3 second jitter between Trends seed calls to reduce throttling risk.

Follow-up 2026-06-29 standalone Pinterest source plan:
- The third source note is useful and now informs `pipeline/scraper.js`. Each niche has three Pinterest search terms in `pinterest_terms`: Fix It = `how to fix`, `home repair hacks`, `DIY repairs`; Cook It = `how to cook`, `easy dinner recipes`, `quick meals`; Grow It = `how to grow vegetables`, `garden tips`, `indoor plants`; Money = `how to save money`, `side hustle ideas`, `money tips`; Get Fit = `home workout`, `how to lose weight`, `fitness tips`; Make It = `DIY crafts`, `how to make`, `upcycle ideas`; Tech It = `phone hacks`, `AI tools`, `productivity tips`; Look Good = `skincare routine`, `how to style outfits`, `grooming tips`.
- Pinterest scraping should assume the manual login pause has already allowed login. For each term, load Pinterest, dismiss register/login/closeup modals with Escape, click the data-test-id search input when available, type slowly with `{ delay: 80 }`, capture autocomplete suggestions from data-test-id suggestion elements, then submit the search and scrape pin titles from data-test-id pin/title selectors after one `window.scrollBy(0, 1500)` and a 2 second wait.
- Use source labels `pinterest_autocomplete` and `pinterest_pins`. Public repin counts are not reliable in search results, so Pinterest engagement remains `0`; the value is intent/validation and cross-source presence rather than numeric engagement.
- Filter empty/null pin titles, warn if fewer than `pinterestMinPinTitlesPerTerm` non-empty titles are found, use 1.5-2.5 second jitter between search terms, and keep Pinterest failures scoped to the current term/source.
- Raw topic dedupe now merges exact duplicate titles into one item while preserving `sources`, `source_count`, `signal_count`, and a `signals` array. This prevents autocomplete/pin/cross-source overlap from being thrown away before Claude scoring.

Follow-up 2026-06-29 standalone YouTube autocomplete source plan:
- The fourth source note is useful and now informs `pipeline/scraper.js`. YouTube should be autocomplete-only, not search-result scraping, because suggestions are cleaner search-demand data and results are noisier creator/SEO artifacts.
- Each niche has three `youtube_seeds`: Fix It = `how to fix`, `how to repair`, `DIY how to`; Cook It = `how to cook`, `how to make`, `easy recipe`; Grow It = `how to grow`, `how to plant`, `gardening how to`; Money = `how to make money`, `how to save money`, `side hustle`; Get Fit = `how to lose weight`, `how to build muscle`, `home workout`; Make It = `how to make`, `how to build`, `DIY how to make`; Tech It = `how to use`, `how to set up`, `ChatGPT how to`; Look Good = `how to style`, `how to do skincare`, `how to`.
- Use a fresh Playwright browser context for YouTube by default (`youtubeUseFreshContext: true`) so suggestions are less personalized by the logged-in Pinterest/Facebook profile. If the main browser is a persistent context, launch a temporary Chromium context and close it after the niche.
- For each seed, stay on YouTube, clear the search box, type the seed with a trailing space and `{ delay: 60 }`, wait up to 8 seconds for `ytd-search-suggestion-renderer`, scrape `span.sbqs_c` first with `#suggestion-text` fallback, and skip `ytd-search-refinement-card-renderer`/trending inserts by querying only suggestion renderers.
- Use source label `youtube_autocomplete`, engagement `0`, and preserve `engagement_detail.seed`, `kind: autocomplete`, and suggestion `position`. Use 800-1200ms jitter between passes and 2000ms between niches; low suggestion counts are acceptable and should not trigger retries.

Follow-up 2026-06-29 standalone Quora source plan:
- The final source note is useful and now informs `pipeline/scraper.js`. Quora is especially valuable for hook writing because its titles are naturally phrased as reader curiosity gaps.
- Each niche now has two `quora_topics` and two `quora_search_terms`: Fix It = topics `Home-Repair`, `DIY-Projects`, searches `how to fix at home`, `home repair tips`; Cook It = `Cooking`, `Recipes`, searches `how to cook`, `easy cooking tips`; Grow It = `Gardening`, `Indoor-Plants`, searches `how to grow plants`, `gardening beginners`; Money = `Personal-Finance`, `Making-Money`, searches `how to make money`, `save money tips`; Get Fit = `Fitness`, `Weight-Loss`, searches `how to lose weight`, `home workout tips`; Make It = `DIY-Projects`, `Arts-and-Crafts`, searches `how to make DIY`, `craft ideas home`; Tech It = `Technology`, `Artificial-Intelligence`, searches `how to use AI tools`, `tech tips beginners`; Look Good = `Skin-Care`, `Fashion-and-Style`, searches `skincare routine tips`, `how to dress better`.
- Quora URL patterns are `https://www.quora.com/topic/{Topic-Name}` and `https://www.quora.com/search?q={term}&type=question`. For each pull, dismiss modal/login overlays with Escape, wait up to 12000ms for question-like selectors, do one scroll pass, and scrape `.q-text.qu-dynamicFontSize--regular` with fallbacks `[class*="question_title"]`, `h2`, and `[data-testid="question-text"]`.
- Filter aggressively: keep only items ending in `?`, under 120 characters, and starting with an uppercase letter. This avoids answer-snippet bleed. Quora engagement remains `0`; source labels are `quora_topic` and `quora_search`, with label/kind/position preserved in `engagement_detail`.
- Quora is sensitive, so pace it separately: 3000ms between pulls, 5000ms between niches, and a 10000ms cooldown after every 2 niches. If CAPTCHA/bot-check text or selectors appear, log a warning, skip that niche, and continue rather than retrying.
- The scorer prompt should explicitly treat Quora questions as pre-validated curiosity-gap phrasing and weight them highly when writing Facebook hooks.

Follow-up 2026-06-29 standalone DB/state spine:
- User correctly noted the standalone how-to module needs a DB to store states, manage statuses, and support resume behavior like engagement/promo modules. JSON files are no longer intended as the source of truth; they are exports/debug snapshots.
- Added `pipeline/db.js` using sql.js and standalone `pipeline/howto-content.sqlite`. Keep this DB isolated from the Nollywood Electron SQLite DB and from existing `social_posts`; do not couple how-to state to Nollywood `project_id`, shorts, characters, or scene assets.
- Current schema tables: `runs`, `source_pulls`, `raw_topics`, `topic_signals`, `scored_topics`, `posts`, `events`, and `schema_meta`. `raw_topics` stores merged topic rows; `topic_signals` stores evidence details; `posts` is the execution queue for later image generation and Facebook scheduling.
- Current lifecycle: run statuses `created/scraping/scoring/queueing/done/failed`; source pulls `pending/running/done/failed`; scored topics `scored`; posts currently start at `queued` and are designed to later progress through `image_generating/image_done/content_done/scheduling/scheduled` plus failure states.
- Startup recovery resets interrupted `source_pulls.running` back to `pending`, resets posts stuck in `image_generating`/`scheduling` back to `queued`, and marks interrupted runs as `failed`. Later adapters should extend this with file-existence proof for generated images and Facebook proof for scheduled posts.
- `scraper.js`, `scorer.js`, and `queue.js` now accept `{ db, runId }`: scraper saves raw topics/signals to DB then exports `raw_topics.json`; scorer reads raw topics from DB when present, saves scored topics to DB then exports `scored_topics.json`; queue reads scored topics from DB when present, upserts `posts`, then exports `posts_queue.json`.
- DB writes create pre-operation backups under `pipeline/backups/`. `pipeline/.gitignore` excludes SQLite files and backups. Validation included schema creation/open/save with a temporary DB smoke test.

Follow-up 2026-06-29 standalone smoke-test filters:
- Added CLI filters for safe source-by-source live tests before full weekly runs. Use `--niche <id-or-name>` to limit pages and `--sources <csv>` to limit scraper sources. Supported sources are `reddit`, `google_trends`, `pinterest`, `youtube`, and `quora`; aliases include `trends`/`google` for `google_trends` and `yt` for `youtube`.
- Filters are applied before DB run creation, and the selected niche/source set is persisted in `runs.options_json`. `scraper.js` only executes enabled source definitions when `config.scraper.sources` is present.
- Recommended first live smoke command: from `pipeline/`, run `npm.cmd run run -- --niche fix-it --sources youtube --skip-score --skip-queue --no-login-pause`. Then inspect DB `raw_topics`, `topic_signals`, `source_pulls`, and exported `raw_topics.json` before testing Reddit/Trends/Pinterest/Quora.

Follow-up 2026-06-29 YouTube smoke proof:
- First live YouTube smoke for `fix-it` proved the DB/run/source-pull flow worked but returned 0 items because the YouTube suggestion DOM no longer matched the initial narrow `ytd-search-suggestion-renderer span.sbqs_c` assumptions.
- `scraper.js` was hardened with broader YouTube wait/extraction selectors (`ytd-search-suggestion-renderer`, `yt-searchbox-suggestion`, `ytSuggestionComponentSuggestion`, ARIA listbox/options), visible search input selectors, post-type settle delay, and compact zero-result diagnostics that report selector counts, input values, title, URL, and body/sample text.
- Second live smoke captured 36 items but exposed duplicated titles because multiple child nodes repeated the same suggestion text. Extraction now uses the first distinct suggestion text per renderer rather than joining all child nodes.
- Final live smoke run `20260629205833_7deb16ab` succeeded: `source_pulls` has `fix-it/youtube` status `done`, `item_count = 36`, DB has 36 `raw_topics` and 36 `topic_signals`, and `raw_topics.json` contains clean titles such as `how to fix stick drift ps5`, `how to fix zipper that came off track`, and `how to fix hole in drywall`.

Follow-up 2026-06-29 Reddit smoke proof:
- Re-running the initial `fix-it` Reddit smoke before fixes reproduced the blocker: `reddit:DIY` returned 0, then a later subreddit aborted the whole Reddit source with `page.evaluate: TypeError: Cannot read properties of undefined (reading 'replace')` inside the DOM metric parser.
- `scraper.js` now treats per-subreddit failures as non-fatal, hardens DOM upvote/comment parsing against missing capture groups, and falls back to Reddit `/top.json?t=month` via Playwright when the rendered DOM returns no usable rows. Reddit signal metadata records `upvotes`, `comments`, `scrape_mode`, `subreddit`, `subreddit_group`, and `min_upvotes`.
- Passing live smoke command from `pipeline/`: `npm.cmd run run -- --niche fix-it --sources reddit --skip-score --skip-queue --no-login-pause`. Run `20260629210607_6bfdb0e1` completed with `source_pulls` status `done`, `item_count = 56`, and DB counts of 56 `raw_topics` plus 56 `topic_signals`.
- The successful run produced primary subreddit rows from `HomeImprovement` and `fixit`, opened backups because primaries were below the 40-item threshold, and added `handyman` backup rows. Sample raw titles included `Dropped my fridge while moving it. Any way to fix it?`, `My Grandma paid $25,000 for new gutters was she scammed?`, and `Can I walk on these steel studs?`.

Follow-up 2026-06-29 Google Trends smoke proof:
- Initial `fix-it` Trends smoke completed but exposed two cleanup issues: the generic seed `how to fix` did not render related queries within 15000ms, and Google row text included leading rank numbers such as `1 habitat...`. `scraper.js` now adds a short post-miss scroll/wait before fallback and strips leading rank numbers from Trends titles in both section and broad-fallback extraction.
- Passing live smoke command from `pipeline/`: `npm.cmd run run -- --niche fix-it --sources google_trends --skip-score --skip-queue --no-login-pause`. Run `20260629211036_7047acf1` completed with `source_pulls` status `done`, `item_count = 1`, DB count of 1 `raw_topics`, and 2 `topic_signals`.
- Treat this source as working but naturally low-yield/volatile. The clean sample was `habitat for humanity home repair program`, backed by both Rising and Top `home repair` related-query signals at `+70%`. Continue to fail soft when a seed does not render; Trends should supplement Reddit/YouTube/Pinterest/Quora rather than block the weekly run.

Follow-up 2026-06-29 timed login hold:
- Pinterest smoke without login pause ran cleanly but returned 0 rows, likely because login/session or selector state needs a real browser check. The previous manual login pause depended on interactive stdin, which is often unavailable from Codex tool runs.
- `pipeline/run.js` now accepts `--login-wait-sec <seconds>` / `--login-wait-ms <milliseconds>`, and `pipeline/config.js` also supports `CONTENT_RESEARCH_LOGIN_WAIT_MS`. If either is set, `scraper.js` opens a holding page with source login links and waits the fixed duration even when stdin is non-interactive.
- Use this for login-sensitive smokes, for example: `npm.cmd run run -- --niche fix-it --sources pinterest --skip-score --skip-queue --login-wait-sec 120`. Do not combine with `--no-login-pause` unless deliberately testing no-login behavior; a positive timed wait still holds the browser open.
- For sessions that should survive across runs, use `--user-data-dir .browser-profile` or set `CONTENT_RESEARCH_USER_DATA_DIR`. `pipeline/.gitignore` excludes `.browser-profile/` and `browser-profile/`.

Follow-up 2026-06-29 Pinterest smoke proof:
- Pinterest no-login smoke run `20260629211235_77a20926` completed but returned 0 rows. After adding a timed login hold and broadening pin extraction to image alt/title/aria text, live run `20260629211727_3519d9e6` produced 120 rows before dedupe and 113 unique raw topics for `fix-it`.
- The successful command was `npm.cmd run run -- --niche fix-it --sources pinterest --skip-score --skip-queue --login-wait-sec 120`. It still warned that autocomplete was unavailable because the search input selector did not appear, then fell back to direct results URLs and scraped pin titles successfully.
- `scraper.js` now filters Pinterest accessibility filler such as `Image may contain:` / `This may contain:` and strips non-ASCII mojibake from pin-title candidates before saving raw topics. Future improvement: find the current logged-in Pinterest search input/typeahead selector so autocomplete intent signals work too, but pin-title validation is already usable.
- Follow-up persistent-profile smoke `20260629212713_7264fd25` used `--user-data-dir .browser-profile --login-wait-sec 10` and produced 104 rows before dedupe / 99 unique raw topics. Pinterest landed on `ca.pinterest.com/business/hub/`, so autocomplete still fell back, but direct search URL pin-title scraping remained productive. Extra filters now drop ad-slot labels such as `3p-google-ad-slot`.

Follow-up 2026-06-29 Quora smoke proof:
- User logged into Quora in the persistent research browser profile. Quora run `20260629213007_f30d2159` produced 49 raw items / 49 unique for `fix-it` with topic pages and searches active.
- No-wait persistent-profile verification run: `npm.cmd run run -- --niche fix-it --sources quora --skip-score --skip-queue --user-data-dir .browser-profile --no-login-pause`. Run `20260629213402_5e5b8357` completed with `source_pulls` status `done`, `item_count = 32`, and 32 unique raw topics after text normalization.
- `scraper.js` now normalizes readable raw text globally: smart quotes/dashes become ASCII, non-breaking spaces become spaces, and emoji is stripped. This avoids terminal/JSON display noise while preserving the useful question phrasing.
- Quora can return off-niche or high-stakes "fix at home" questions such as dental/body topics. Do not overfit raw scraping around one sample; the scorer prompt now explicitly rejects unsafe, illegal, high-risk medical/legal/financial, off-niche, ad-label, UI-label, and image-alt-text topics before queueing.

Follow-up 2026-06-29 first end-to-end one-page proof:
- First full `fix-it` run using all five sources succeeded with command `npm.cmd run run -- --niche fix-it --user-data-dir .browser-profile --no-login-pause`. Run `20260629213534_264c8a3c` scraped 275 unique raw topics, scored 30, and queued 30 posts. Source counts in that run: Reddit 56, Google Trends 13, Pinterest 119 before dedupe, YouTube 36, Quora 55.
- The first score/queue output included several risky repair-adjacent topics (electrical wiring, garage door springs, fuel/vehicle internals). `scorer.js` prompt was tightened to prefer low-risk household fixes and exclude procedural dangerous steps around live electrical wiring, garage springs, gas/fuel systems, structural supports, vehicle safety systems, toxic chemicals, and appliance internals. Safety-warning/when-to-call-a-pro posts are allowed only when they give no dangerous steps.
- `scorer.js` and `queue.js` now normalize generated text too, including smart punctuation, emoji, and common mojibake sequences such as UTF-8 dashes decoded as `â...`. This prevents generated hooks/prompts from writing corrupted punctuation into `scored_topics.json`, `posts_queue.json`, and DB posts.
- Rescore-only run from the same raw topics succeeded with command `npm.cmd run run -- --niche fix-it --skip-scrape --no-login-pause`. Final run `20260629214338_aec66e90` is `done` in DB with 275 `raw_topics`, 30 `scored_topics`, and 30 `posts`; artifacts `scored_topics.json` and `posts_queue.json` each contain 30 items. Post-QA found no mojibake/smart-punctuation leftovers and the only rough high-risk regex hit was a false positive on zipper "teeth".
- Final top queue examples: running toilet water-waste fix, drywall hole patching, stripped screw removal, broken zipper repair, and water-stain primer. This is the current best known one-page baseline before running all eight pages.

Follow-up 2026-06-29 all-eight-page dry-run proof:
- The first all-eight run `20260629215050_52200183` exposed a Pinterest hang near the final `look-good` term after autocomplete warnings. `scraper.js` now runs each Pinterest term in a temporary page, suppresses Pinterest autocomplete globally after the first missing-input proof, and enforces a per-term timeout so one stuck term cannot hold the whole weekly run.
- Passing all-eight command from `pipeline/`: `npm.cmd run run -- --user-data-dir .browser-profile --no-login-pause`. Run `20260629222820_d3f0fcc8` completed successfully at `2026-06-29T22:58:13.532Z` with DB run status `done`, stage `queue`, and no error.
- DB proof for run `20260629222820_d3f0fcc8`: all 40 source pulls are `done` (`8 niches * 5 sources`), `raw_topics` has 2122 unique items, `scored_topics` has 240 rows, and `posts` has 240 queued rows. Artifact exports were written to `pipeline/raw_topics.json`, `pipeline/scored_topics.json`, and `pipeline/posts_queue.json`; logs are in `pipeline/logs/run_2026-06-29.log`; state DB is `pipeline/howto-content.sqlite`.
- Per-niche raw counts from the dry run: Fix It 276, Cook It 274, Grow It 257, Money 253, Get Fit 279, Make It 271, Tech It 284, Look Good 228. Each niche scored and queued exactly 30 posts.
- Artifact QA after the run found no mojibake/smart-punctuation leftovers in scored topics or queued posts. One Tech It queued item used timely wording ("Google just released..."), so current-news/product-specific claims should be source/date reviewed before actual posting.
- No Electron UI is required for the research dry run. A UI may become useful later for monitoring, curation, resume controls, and Facebook scheduling visibility, but the current standalone DB-backed CLI is enough for scrape, score, queue, and resume-state validation.

Follow-up 2026-06-29 queue review gate:
- Added `pipeline/review.js` and `npm.cmd run review -- ...` as the review/approval CLI before any image generation or Facebook scheduling. Core commands: `status`, `list`, `show`, `approve`, `reject`, `needs-review`, `reset`, `set-status`, `auto-flag`, and `export`.
- `posts` now has `review_note` and `reviewed_at` columns. Review statuses are stored in SQLite and exported back to `pipeline/posts_queue.json`, so the JSON queue remains a view of DB state rather than an independent source of truth.
- Bulk status changes require explicit selectors (`--id`, `--niche --rank`, or `--all`) and updates over 10 posts require `--yes`. This is the guardrail before later image-credit spend and Facebook posting.
- `auto-flag` detects current-claim, safety-risk, and high-stakes-advice wording. It was run on all-eight dry-run `20260629222820_d3f0fcc8` and moved four posts to `review_needed`: Cook It rank 21 (`this week`), Make It rank 5 (`today`), Money rank 29 (`today`), and Tech It rank 17 (`Google just released...`). Current post status counts for that run are 236 `queued` and 4 `review_needed`.

Follow-up 2026-06-29 image dry-run stage:
- Added `pipeline/image.js` and `npm.cmd run image -- --dry-run ...`. Live Higgsfield image generation is intentionally disabled in this version; without `--dry-run`, the command exits before doing any browser or credit-spending work.
- Added `image_jobs` table to the standalone DB. Each job stores `run_id`, `post_id`, `provider`, `model`, `mode`, `status`, dry-run flag, prompt, payload JSON, manifest path, output path, and timestamps. Interrupted `image_jobs.generating` rows reset to `pending`; interrupted `posts.image_generating` rows reset to `approved`.
- Image dry-run selects only `posts.status = approved` by default, writes `pipeline/image_manifest.json`, creates expected output folders under `pipeline/generated_images/<run_id>/<niche>/`, and records image jobs with status `dry_run` while leaving the post itself `approved`.
- Seed proof: inspected and approved Fix It rank 1 (`post_id = 91`, running toilet prompt) with note `image dry-run seed`, then ran `npm.cmd run image -- --dry-run --limit 1`. Result: one `image_jobs` row with status `dry_run`, provider `higgsfield`, model `nano-banana`, and expected output path `generated_images\20260629222820_d3f0fcc8\fix-it\01_how-to-fix-a-running-toilet-yourself.png`.
- Current post status counts for all-eight run `20260629222820_d3f0fcc8` after the seed approval are 235 `queued`, 1 `approved`, and 4 `review_needed`. Do not treat `dry_run` image jobs as completed image assets; they are payload/job-state proof only.

Follow-up 2026-06-29 Facebook page-context dry run:
- Added `pipeline/facebook.js` and `npm.cmd run facebook -- --dry-run ...`. Live Facebook posting/scheduling is intentionally disabled in this command; without `--dry-run`, it exits before any browser work that could create or schedule content.
- Added `facebook_page_context_checks` table. Each check stores `run_id`, `niche_id`, target page name, status, active profile proof, dashboard/content-library URL, screenshot path, diagnostics JSON, error, and timestamps. Interrupted `pending/running` checks reset to `failed` on startup.
- First launch from sandbox failed with Playwright `spawn EPERM`; rerun required GUI/browser approval. First login attempt showed the `.browser-profile` was not logged into Facebook; running with `--login-wait-sec 180` allowed manual login, after which `Fix It` verified.
- Initial all-page pass verified 5/8 and failed `Make Money`, `Make It`, and `Tech It` because the profile-panel search selector grabbed the global `Search Facebook` input behind the modal. Screenshot proof also showed the real money page label is `Money it`. Fixes: canonical money `facebook_page_name` changed to `Money it`; aliases still include `Money`/`Make Money`; profile selection now clicks visible rows before search, narrows the search selector to `Search profiles...`, and scrolls the profiles panel if needed.
- Existing standalone DB rows in `raw_topics`, `scored_topics`, and `posts` for `niche_id = money` were updated from `Make Money` to `Money it`, and `posts_queue.json` was re-exported.
- Final all-eight command passed: `npm.cmd run facebook -- --dry-run --all-pages --user-data-dir .browser-profile --login-wait-sec 30`. DB proof rows 11-18 are all `verified`: Fix It, Cook It, Grow It, Money it, Get Fit, Make It, Tech It, and Look Good. Report export: `pipeline/facebook_context_checks.json`; screenshots: `pipeline/facebook_context_diagnostics/0011_*` through `0018_*`.
- Treat URL-only proof as insufficient. The verifier records switch proof from the page home context plus Content Library readiness and account-menu diagnostics. Later scheduling must require a recent `facebook_page_context_checks.status = verified` row for the target page before calling the existing Content Library image-post scheduler.

Follow-up 2026-06-29 Higgsfield live image proof:
- Added a guarded live image path in `pipeline/image.js` plus `pipeline/adapters/higgsfield-image-generator.js`. It reuses the existing `src/main/automation/higgsfield.js` automation from a standalone project dir at `pipeline/_higgsfield/project`, with session state saved at `pipeline/higgsfield-session.json`.
- Live mode is intentionally one-post gated: use `npm.cmd run image -- --live --limit 1 --login-wait-sec 240` or an exact post selector. Do not batch live image generation until more images have been visually QA'd and the downstream Facebook handoff is proven.
- The adapter checks the Higgsfield session before any generation. If the session is missing/expired and a login wait is provided, it opens the visible browser, holds for manual login, retries preflight, and only proceeds after `verifySession()` passes. This was used successfully for the first run.
- First live proof command: `npm.cmd run image -- --live --limit 1 --login-wait-sec 240` from `pipeline/`. The user logged into Higgsfield during the wait, preflight passed, and exactly one image Generate click occurred for post `#91` / Fix It rank 1 ("How to fix a running toilet yourself").
- Credit/proof metadata for image job `#1`: `gen_clicked_at = 2026-06-30T00:48:21.500Z`, `credit_cost = 2`, `source_gen_id = 8a2adbd0-a01c-4347-a822-ed1ee158ef85`, prompt-matched Asset Library download similarity `100`, generation duration `114792ms`, status `done`.
- Output file: `pipeline/generated_images/20260629222820_d3f0fcc8/fix-it/01_how-to-fix-a-running-toilet-yourself.png`, about 9.5 MB. Visual QA showed a usable square, text-free toilet-tank repair image. Post `#91` is now `image_done` with `image_path` and `generated_at = 2026-06-30T00:49:40.089Z`.
- Current queue state after the first live image: run `20260629222820_d3f0fcc8` remains `done/queue`; posts are `image_done = 1`, `queued = 235`, `review_needed = 4`. No Facebook post/schedule action has been performed.

Follow-up 2026-06-29 standalone Facebook scheduling handoff:
- Added guarded Facebook scheduling state to the standalone module. `pipeline/db.js` schema version is now 6 with `facebook_schedule_jobs`, tracking `post_id`, target page, dry-run/live status, image path, caption, scheduled date/time, context check id, Facebook post id, screenshot, errors, and timestamps.
- `pipeline/facebook.js` now has explicit modes: `--dry-run` for page-context verification only, `--prepare` for local caption/date/time preparation, `--schedule-dry-run` for DB/file/context-gated handoff proof, and `--live` for one-post scheduling through the existing `SocialFacebookUploader`.
- Live scheduling is still one-post gated (`--live --limit 1` or exact `--id` / `--niche --rank`) and re-verifies the target page in the same browser session immediately before handing the image/caption/date/time into the engagement/promo uploader. If failure happens before the actual scheduling attempt, the post falls back to `content_done`; if the uploader cannot confirm after an attempt, it marks the post failed for manual inspection rather than auto-retrying.
- Prepared post `#91` with `npm.cmd run facebook -- --prepare --id 91 --next-slot`. It moved from `image_done` to `content_done`, with caption `Your toilet is silently adding dollars to your water bill every single day...`, scheduled date `2026-06-30`, scheduled time `15:00`, and image path `generated_images\20260629222820_d3f0fcc8\fix-it\01_how-to-fix-a-running-toilet-yourself.png`.
- Schedule dry-run proof passed with `npm.cmd run facebook -- --schedule-dry-run --id 91`. `facebook_schedule_jobs` row `#1` is `status = dry_run`, `dry_run = 1`, target page `Fix It`, context check id `11`, no error. The context proof row `#11` is still `verified` for `Fix It`, completed `2026-06-30T00:34:22.641Z`, screenshot `facebook_context_diagnostics\0011_Fix_It_verified.png`.
- No live Facebook scheduling/posting has been run yet. Current post state is `#91 = content_done`, ready for a one-post live test command only if the user explicitly wants the real schedule action: `npm.cmd run facebook -- --live --id 91 --user-data-dir .browser-profile --login-wait-sec 180`.

Follow-up 2026-06-29 first live Facebook schedule proof:
- User explicitly approved going all the way for the first Facebook live scheduling test and said they would delete the post later if needed.
- Initial live attempt exposed a serious page-context false positive: the browser was on `Tech It` while the verifier accepted `Fix It` because the page name appeared somewhere in the UI. No final schedule click occurred in that bad-context attempt. Fix: `pipeline/facebook.js` now treats generic dashboard/name presence as insufficient and hard-gates on Content Library readiness plus account-menu active target proof after switching. The verifier must switch through Account -> See all profiles -> target page if needed, then confirm the target remains active before scheduling.
- Additional Facebook UI fixes from live testing: `src/main/social/social-facebook-uploader.js` now requires a real visible Create Post dialog before uploading, chooses the visible composer dialog for hydration, dismisses hashtag suggestion overlays after caption entry, clicks the image-post `Next` button before scheduling, clicks the current `Scheduling options` row, and accepts the final blue button labeled `Schedule` as well as legacy `Schedule post`.
- Failed pre-submit attempts were reset only after DB/screenshot proof showed no scheduled row and no final schedule confirmation. The final successful run was `npm.cmd run facebook -- --live --id 91 --user-data-dir .browser-profile --login-wait-sec 180`.
- Successful live proof output: page context verified as `Fix It`, image uploaded, caption entered, `Next` clicked, date set to `2026-06-30`, time set to `15:00`, `Schedule for later` clicked, final `Schedule` clicked, 30s settle completed, and scheduled row confirmed by in-place row match with `caption+time`.
- DB proof after success: post `#91` status `scheduled`, page `Fix It`, `scheduled_date = 2026-06-30`, `scheduled_time = 15:00`, `scheduled_at = 2026-06-30T01:34:39.991Z`, no error. `facebook_schedule_jobs` row `#1` status `done`, `dry_run = 0`, `context_check_id = 24`, no error. Run `20260629222820_d3f0fcc8` post counts are now `scheduled = 1`, `queued = 235`, `review_needed = 4`.
- The successful scheduled post is a real Facebook scheduled image post on the `Fix It` page. If this remains only a test, the user may delete it manually from Facebook. Do not rerun live scheduling for post `#91` unless it is reset deliberately, because the DB now marks it scheduled.

Follow-up 2026-06-29 standalone content-quality layer:
- The first live scheduled Facebook post `#91` proved the automation path, not the content quality path. The image upload, page-context verification, schedule modal flow, final Schedule click, and Content Library confirmation all worked, but the caption was only thin hook/topic placeholder copy and did not actually solve the running-toilet problem. Treat this post as a test/proof that the user intends to delete on Facebook, not as production content.
- Added a standalone content stage for the how-to module. The intended post flow is now `image_done -> content_generating -> content_done -> scheduling -> scheduled`. `pipeline/content.js` writes `content_jobs`, can run `--dry-run` prompt manifests without calling Claude, and `--generate` creates practical Facebook copy with a hook, tools/materials, 3-5 numbered steps, a safety/pro note, CTA, and hashtags.
- Added a caption quality guard in `pipeline/content-quality.js`. Facebook `--prepare`, `--schedule-dry-run`, and `--live` now refuse thin captions by default when they are too short, missing tools/materials, missing 3+ actionable numbered steps, or missing a safety/caution/pro note. `--allow-thin-caption` exists only for deliberate automation tests.
- `pipeline/facebook.js` no longer builds fallback placeholder captions from hook/topic. `--prepare` only assigns `scheduled_date`/`scheduled_time` to existing `content_done` posts whose caption passes the guard. To generate a replacement for the #91 topic, use `npm.cmd run content -- --generate --id 91` only after resetting/selecting an image-done replacement row, then schedule after review.

Follow-up 2026-06-30 standalone content credential proof:
- User confirmed the standalone module should reuse the Claude key from the Nollywood Electron app. `pipeline/credentials.js` now loads local `.env` files and checks multiple Electron-store paths, including `%APPDATA%\nollywood-ai-pipeline\config.json`, for `claudeApiKey` and common Anthropic key field aliases. Do not print key values.
- In Codex sandboxed tool runs, reading `%APPDATA%\nollywood-ai-pipeline\config.json` is blocked unless the command is run with approval/unsandboxed access. An approved metadata check confirmed `claudeApiKey` exists in that config. Normal local terminal runs should not need this Codex approval layer.
- To preserve the proof trail, do not mutate scheduled proof post `#91` into a replacement. A replacement row `#301` was created from the same Fix It topic/image, leaving `#91` scheduled/test-marked. Post `#301` then generated Claude content successfully with model `claude-sonnet-4-6`, passed the quality guard, and schedule dry-run passed for `Fix It` on `2026-07-01 15:00`.

Follow-up 2026-06-30 standalone Facebook replacement live proof:
- User deleted the dummy/proof Facebook post `#91`; the standalone DB now marks post `#91` as `deleted` with an audit note, so it no longer inflates production scheduled counts.
- Live Facebook scheduling for replacement post `#301` succeeded with command `npm.cmd run facebook -- --live --id 301 --user-data-dir .browser-profile --login-wait-sec 180`. The flow re-verified `Fix It` page context, uploaded `generated_images\20260629222820_d3f0fcc8\fix-it\01_how-to-fix-a-running-toilet-yourself.png`, entered the 1402-char Claude caption, set date `2026-07-01` and time `15:00`, clicked final `Schedule`, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#301` status `scheduled`, page `Fix It`, `scheduled_date = 2026-07-01`, `scheduled_time = 15:00`, `scheduled_at = 2026-06-30T04:35:35.649Z`, no error. `facebook_schedule_jobs` row `#2` status `done`, `dry_run = 0`, `context_check_id = 25`, no error. Current all-eight run counts should be `scheduled = 1`, `deleted = 1`, `queued = 235`, and `review_needed = 4`.

Follow-up 2026-06-30 standalone duplicate guard:
- Added `pipeline/dedup.js` plus DB schema version 8 fields on `posts`: `topic_key`, `content_fingerprint`, `duplicate_of_post_id`, `duplicate_reason`, and `dedupe_checked_at`. Existing rows were backfilled with `npm.cmd run dedup -- --backfill`.
- Fingerprints are topic-first token sets with light normalization/stemming/synonyms, so `How to fix a running toilet yourself` fingerprints as `run toilet`. The checker correctly hard-matches a future variant like `How to stop a toilet from running` against scheduled post `#301` with score about `0.816`, while `npm.cmd run dedup -- --id 301` does not match the post to itself. Deleted/test post `#91` is ignored by default because `deleted` is not in the duplicate status set.
- Queue-time behavior: hard repeats of already scheduled/published posts become `duplicate`; softer matches become `review_needed`. Final Facebook `--prepare`, `--schedule-dry-run`, and `--live` also call the duplicate guard against scheduled/published history and require `--allow-duplicate` for deliberate manual repeats.
- `PipelineDb.save()` now uses process-unique temp files (`.pid.timestamp.tmp`) to avoid races when multiple read/check commands open and save the standalone SQLite DB concurrently.

Follow-up 2026-06-30 standalone second-page live proof:
- Second-page smoke proved the standalone module is not Fix-It-specific. Chosen post was Cook It `#62`, topic `How to make homemade butter in minutes`, after dedup returned no prior match. It was approved with note `Cook It second-page live smoke seed`.
- Higgsfield live image generation for `#62` succeeded using existing session, model `nano-banana`, cost `2` credits, source generation id `eb26edc2-b950-48b3-83df-da108992a66b`, and 100% prompt-matched Asset Library recovery. Output file: `generated_images\20260629222820_d3f0fcc8\cook-it\02_how-to-make-homemade-butter-in-minutes.png`. Visual QA showed a clean, text-free butter image on a wooden board.
- Initial Claude content generation produced useful copy but failed the local validator because the food-specific `Quick check` spoilage note was not recognized as a safety note. `pipeline/content-quality.js` now treats `quick check`, `discard`, `spoil/spoiled`, `food safety`, `refrigerate`, `raw`, and `undercooked` as valid safety/caution signals. Rerun passed with 1467-char caption and 4 actionable steps.
- Cook It page context dry-run verified successfully. Schedule dry-run passed. Live command `npm.cmd run facebook -- --live --id 62 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Cook It, uploaded the butter image, entered caption, set `2026-07-01 16:00`, clicked final `Schedule`, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#62` status `scheduled`, page `Cook It`, `scheduled_date = 2026-07-01`, `scheduled_time = 16:00`, `scheduled_at = 2026-06-30T05:13:09.199Z`, no error. `image_jobs` row `#2` done with credit cost `2`; `facebook_schedule_jobs` row `#3` done, `dry_run = 0`, `context_check_id = 27`, no error. Current all-eight run counts: `scheduled = 2`, `deleted = 1`, `queued = 234`, `review_needed = 4`.

Follow-up 2026-06-30 standalone weekly planner:
- Added `pipeline/weekly.js` and `npm.cmd run weekly -- --dry-run`. This is a read-only planner, not a live batch runner.
- The planner selects the next eligible queued posts per page, skips pages that already have active posts in the current run, runs the duplicate guard before selection, avoids already occupied scheduled/scheduling time slots, writes `pipeline/weekly_plan.json`, and prints the exact next commands for approval, image, content, page-context, schedule dry-run, and one-post live scheduling.
- Keep this dry-run-only until more pages have been live-proven. Do not batch image generation or Facebook scheduling from the weekly plan without explicit user approval and one-post monitoring.
- Later same day, the planner was upgraded from a one-post smoke planner to a 7-day calendar planner. Default `npm.cmd run weekly -- --dry-run` now plans `days = 7` and `postsPerPage = 7`, marks existing scheduled/scheduling/content-done posts in the date window as `already_scheduled`/`prepared`, fills empty requested slots from queued posts, dedups against DB history and the planned batch, and keeps fixed page slots from config order: Fix It 15:00, Cook It 16:00, Grow It 17:00, Money it 18:00, Get Fit 19:00, Make It 20:00, Tech It 21:00, Look Good 22:00. Use `--days 1` for the old one-day smoke view.

Follow-up 2026-06-30 standalone Grow It live proof:
- First weekly-plan continuation post was Grow It `#151`, topic `Common myths about keeping rabbits out of the garden`. It was approved from `queued` with note `weekly plan grow-it one-post live path`.
- Higgsfield live image generation succeeded using the existing session. The image job `#3` used model `nano-banana`, cost `2` credits, captured source generation id `de6e70f8-fe5e-42e5-940d-73bc0f2ebc46`, and recovered from Asset Library with 100% prompt similarity after the direct API endpoint returned 404s. Output file: `generated_images\20260629222820_d3f0fcc8\grow-it\01_common-myths-about-keeping-rabbits-out-of-the-garden.png`. Visual QA showed a clean text-free garden image with a rabbit among vegetables.
- Claude content generation passed the useful-caption guard on first run. Grow It Facebook page-context dry-run verified successfully, then `--prepare` set `scheduled_date = 2026-07-01` and `scheduled_time = 17:00`, and `--schedule-dry-run` passed.
- Live command `npm.cmd run facebook -- --live --id 151 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Grow It, uploaded the rabbit/garden image, entered the generated caption, set `2026-07-01 17:00`, clicked final `Schedule post`, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#151` status `scheduled`, page `Grow It`, `scheduled_date = 2026-07-01`, `scheduled_time = 17:00`, `scheduled_at = 2026-06-30T17:51:06.134Z`, no error. `facebook_schedule_jobs` row `#4` status `done`, `dry_run = 0`, `context_check_id = 29`, no error. Current all-eight run counts after refreshing status: `scheduled = 3`, `deleted = 1`, `queued = 233`, `review_needed = 4`.
- Refreshed `pipeline/weekly_plan.json` after this live proof; it now skips Fix It, Cook It, and Grow It, and proposes the remaining five pages: Money it `#241` at 18:00, Get Fit `#121` at 19:00, Make It `#211` at 20:00, Tech It `#271` at 21:00, and Look Good `#181` at 22:00 on `2026-07-01`.

Follow-up 2026-06-30 standalone post/image QA guard:
- Added `pipeline/qa.js` and `npm.cmd run qa -- --dry-run|--run`. This is now the generic production guard between content generation and Facebook scheduling.
- DB schema version is now 9 with `post_quality_checks` plus `posts.quality_verdict` and `posts.quality_checked_at`. QA statuses are `dry_run`, `running`, `passed`, `review_needed`, `blocked`, and `failed`; post flow is now `content_done -> qa_generating -> qa_done -> scheduling -> scheduled`.
- QA dry-run records local checks only: image existence/size/dimensions/aspect, caption guard metrics, and topic/caption term overlap. QA run uses the existing Claude key path, sends the generated image plus topic/caption to Claude vision/text review, and requires a structured verdict. `pass` moves the post to `qa_done`; `review_needed` or `blocked` moves the post to `review_needed` with the QA reason.
- Facebook `--prepare`, `--schedule-dry-run`, and `--live` now default-select `qa_done` instead of `content_done`, and hard-require the latest `post_quality_checks` row to be `passed/pass` with matching `image_path` and caption SHA-256 hash. `--allow-qa-override` exists only for deliberate manual tests. A negative test against scheduled Grow It `#151` with only a QA `dry_run/local_ok` row correctly failed the Facebook schedule dry-run gate.
- `pipeline/weekly.js` now includes `post_image_qa` in each selected post's required action list, between `generate_content` and `verify_facebook_page`. Regenerated `pipeline/weekly_plan.json` after this change.
- Follow-up hardening while running Money it: `pipeline/content.js` now uses Claude tool output (`write_howto_caption`) instead of relying on free-text JSON, because Money it `#241` twice failed on malformed JSON before the patch. The content prompt now targets a buffer below the hard max length, forbids named apps/products/websites/organizations unless present in source signals, and forbids emoji-dependent CTAs. `pipeline/content-quality.js` now flags broken CTA text like `Drop a below`.
- `pipeline/qa.js` now converts large PNG/WebP images to a 1200px JPEG for Claude vision when needed, using ffmpeg if available, and enforces a 5MB base64 ceiling. This avoids Higgsfield's 2K PNG files failing the vision request size limits.

Follow-up 2026-06-30 standalone Money it live proof with QA gate:
- Money it `#241`, topic `How to save money fast on a low income`, was the first post scheduled through the new QA-required path. It was approved from `queued` with note `weekly plan money-it one-post live path`.
- Higgsfield image generation succeeded using existing session, model `nano-banana`, cost `2` credits, source generation id `eba8f467-64d3-4abf-817b-6dde244cbfc8`, and prompt-matched Asset Library recovery at 100% similarity. Output file: `generated_images\20260629222820_d3f0fcc8\money\01_how-to-save-money-fast-on-a-low-income.png`. Visual QA showed a clean, text-free jar/coins/bills savings image.
- Initial content generation exposed malformed JSON output; after switching content generation to structured Claude tool output it passed. A first QA pass found a minor broken CTA (`Drop a below`) and named/stale app risk in the caption, so content prompt and local guard were hardened and the caption was regenerated. Final caption passed content guard at 1178 chars.
- Real QA command `npm.cmd run qa -- --run --id 241` passed with score `88`, verdict `pass`, and moved the post to `qa_done`. QA reasons included image/topic match, no watermark/text overlay/artifacts, actionable caption, and acceptable safety.
- Money it page context dry-run verified successfully. Facebook `--prepare` set `scheduled_date = 2026-07-01`, `scheduled_time = 18:00`; `--schedule-dry-run` passed; live command `npm.cmd run facebook -- --live --id 241 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Money it, uploaded the image, entered caption, set the scheduled time, clicked final Schedule, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#241` status `scheduled`, page `Money it`, `scheduled_date = 2026-07-01`, `scheduled_time = 18:00`, `scheduled_at = 2026-06-30T19:10:47.561Z`, `quality_verdict = pass`, no error. `post_quality_checks` row `#2` status `passed`, verdict `pass`, score `88`. `facebook_schedule_jobs` row `#5` status `done`, `dry_run = 0`, `context_check_id = 31`, no error. Current all-eight run counts: `scheduled = 4`, `deleted = 1`, `queued = 232`, `review_needed = 4`.
- Refreshed `pipeline/weekly_plan.json` after this live proof; it now has `already scheduled = 4`, `ready = 52`, and the next operational post is Get Fit `#121` for `2026-07-01 19:00`.

Follow-up 2026-06-30 standalone Get Fit live proof with QA gate:
- Get Fit `#121`, topic `How walking alone can drive serious weight loss`, was approved from `queued` with note `weekly plan get-fit one-post live path`.
- Before generating/scheduling, the fitness prompt and QA layer were hardened for health-advice drift: captions must avoid guaranteed pounds/calorie burn/timeline promises, avoid replacing medical care, recommend gradual progression/rest, and include stop/seek-help caveats for chest pain, dizziness, unusual shortness of breath, sharp pain, chronic conditions, heart conditions, pregnancy, or injury.
- Higgsfield image generation succeeded using existing session, model `nano-banana`, cost `2` credits, source generation id `940ef22e-7ba2-417e-a034-b7fdc9978d5c`, and prompt-matched Asset Library recovery at 100% similarity. Output file: `generated_images\20260629222820_d3f0fcc8\get-fit\01_how-walking-alone-can-drive-serious-weight-loss.png`. Visual QA showed a clean, text-free walking-path image.
- The first content attempt failed the local caption validator because the action-step detector was too narrow for walking/fitness verbs and the hook leaned too strongly into calorie-burning. `pipeline/content-quality.js` now recognizes fitness action verbs such as `start`, `pick`, `track`, `walk`, and `vary`; `pipeline/content.js` now softens high-stakes Get Fit hooks away from calorie/timeline language. Regenerated caption passed at 1372 chars.
- Real QA command `npm.cmd run qa -- --run --id 121` passed with score `88`, verdict `pass`, and moved the post to `qa_done`. QA reasons included image/topic match, actionable walking steps, beginner-safe gradual progression, safety caveats, no promised pounds/calories/timelines in the caption, and no watermark/text overlay/artifacts.
- Get Fit page context dry-run verified successfully. Facebook `--prepare` set `scheduled_date = 2026-07-01`, `scheduled_time = 19:00`; `--schedule-dry-run` passed; live command `npm.cmd run facebook -- --live --id 121 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Get Fit, uploaded the image, entered caption, set the scheduled time, clicked final Schedule, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#121` status `scheduled`, page `Get Fit`, `scheduled_date = 2026-07-01`, `scheduled_time = 19:00`, `scheduled_at = 2026-06-30T19:43:25.628Z`, `quality_verdict = pass`, no error. `post_quality_checks` row `#3` status `passed`, verdict `pass`, score `88`. `facebook_schedule_jobs` row `#6` status `done`, `dry_run = 0`, `context_check_id = 33`, no error. Current all-eight run counts after refreshing status: `scheduled = 5`, `deleted = 1`, `queued = 231`, `review_needed = 4`.
- Refreshed `pipeline/weekly_plan.json` after this live proof; it now has `already scheduled = 5`, `ready = 51`, and the next operational post is Make It `#211` for `2026-07-01 20:00`.

Follow-up 2026-06-30 standalone Make It live proof with QA gate:
- Make It `#211`, topic `How to make natural cleaning products at home`, was approved from `queued` with note `weekly plan make-it one-post live path`.
- Before content generation, the Make It cleaner rules were hardened: never instruct mixing bleach with vinegar/ammonia/acids/other cleaners, do not combine vinegar and castile soap in the same cleaner, include labeling/ventilation/surface spot-testing/keep-away guidance when relevant, and avoid disinfectant kill claims, mold-remediation claims, pest-control claims, or medical sanitizing guarantees. The local caption guard now distinguishes unsafe cleaner-mixing instructions from explicit warnings like `Never mix vinegar with bleach`.
- First Higgsfield image generation technically succeeded, model `nano-banana`, cost `2` credits, source generation id `4d1b5622-f5e9-4b1d-bf91-437265fdd7e8`, but QA blocked it because the bottles had prominent readable labels (`WHITE VINEGAR`, `BAKING SODA SOLUTION`, etc.), duplicate labeled props, and a visual/caption mismatch. This proved the generic post/image QA guard is catching production-polish failures before Facebook scheduling.
- The image prompt was revised to require unlabeled blank bottles, no words/logos/printed text, no duplicate bottles, and props matching the caption. Replacement Higgsfield generation succeeded with source generation id `d082adae-e484-4f68-8f4a-12c8fb570ec2`, cost `2` credits, and 100% prompt-matched Asset Library recovery. Output file: `generated_images\20260629222820_d3f0fcc8\make-it\01_how-to-make-natural-cleaning-products-at-home.png`. Visual QA showed clean unlabeled cleaner props on a bright counter.
- Regenerated content passed the local caption guard. Real QA command `npm.cmd run qa -- --run --id 211` passed with score `88`, verdict `pass`, and moved the post to `qa_done`.
- Make It page context dry-run verified successfully. Facebook `--prepare` set `scheduled_date = 2026-07-01`, `scheduled_time = 20:00`; `--schedule-dry-run` passed; live command `npm.cmd run facebook -- --live --id 211 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Make It, uploaded the corrected image, entered caption, set the scheduled time, clicked final Schedule, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#211` status `scheduled`, page `Make It`, `scheduled_date = 2026-07-01`, `scheduled_time = 20:00`, `scheduled_at = 2026-06-30T20:19:17.428Z`, `quality_verdict = pass`, no error. `image_jobs` latest row `#6` status `done`, `credit_cost = 2`, `source_gen_id = d082adae-e484-4f68-8f4a-12c8fb570ec2`. `post_quality_checks` latest row `#4` status `passed`, verdict `pass`, score `88`. `facebook_schedule_jobs` row `#7` status `done`, `dry_run = 0`, `context_check_id = 35`, no error.
- Current all-eight run counts after refreshing status: `scheduled = 6`, `deleted = 1`, `queued = 230`, `review_needed = 4`. Refreshed `pipeline/weekly_plan.json`; next operational post is Tech It `#271` for `2026-07-01 21:00`, followed by Look Good `#181` for `2026-07-01 22:00`.

Follow-up 2026-06-30 standalone Tech It live proof with QA gate:
- Tech It `#271`, topic `Free AI tools that replace expensive paid apps`, was approved from `queued` with note `weekly plan tech-it one-post live path`.
- Before image/content generation, Tech It rules were hardened: no `free forever` claims, no `replace every paid app` framing, no named tools/apps/companies unless source-supported, no current/recent launch claims unless sourced, and required caveats around current pricing, terms, privacy settings, data retention, and sensitive uploads. The local caption guard now flags absolute/free-forever tech claims.
- `pipeline/image.js` was also hardened with a universal visual guard: no readable words, labels, logos, app icons, brand marks, UI text, captions, printed text, or brand-like color/geometry; use generic blank props and abstract interface shapes. This came from two Tech It prompt risks: the original queued prompt asked for `AI tool logos`, and the first generated replacement had abstract color blocks that QA judged too Microsoft-like for a post about replacing paid software.
- First Tech It Higgsfield image generation succeeded with source generation id `a9f0f94a-8b49-4b4a-a0fc-dffe3377d3c9`, cost `2` credits, but QA returned `review_needed` because the laptop graphic looked too close to Microsoft-style four-color branding. The caption itself was judged strong, hedged, and privacy-aware.
- The image prompt was revised to a neutral monochrome teal neural-network/chat interface with no brand-like color blocks. Replacement Higgsfield generation succeeded with source generation id `61a29500-b183-4d88-9019-2a3df8b658d1`, cost `2` credits, and 100% prompt-matched Asset Library recovery. Output file: `generated_images\20260629222820_d3f0fcc8\tech-it\01_free-ai-tools-that-replace-expensive-paid-apps.png`. Visual QA showed a clean generic laptop/AI interface image.
- Content generation initially exposed another validator gap: tech workflow steps like `search`, `sign up`, `compare`, `test`, `note`, `review`, `verify`, and `upload` were not counted as actionable step verbs. `pipeline/content-quality.js` now recognizes those verbs. Regenerated content passed the local guard; real QA command `npm.cmd run qa -- --run --id 271` passed with score `88`, verdict `pass`, and moved the post to `qa_done`.
- Tech It page context dry-run verified successfully. Facebook `--prepare` set `scheduled_date = 2026-07-01`, `scheduled_time = 21:00`; `--schedule-dry-run` passed; live command `npm.cmd run facebook -- --live --id 271 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Tech It, uploaded the corrected image, entered caption, set the scheduled time, clicked final Schedule, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#271` status `scheduled`, page `Tech It`, `scheduled_date = 2026-07-01`, `scheduled_time = 21:00`, `scheduled_at = 2026-06-30T23:21:25.395Z`, `quality_verdict = pass`, no error. `image_jobs` latest row `#7` status `done`, `credit_cost = 2`, `source_gen_id = 61a29500-b183-4d88-9019-2a3df8b658d1`. `post_quality_checks` latest row `#5` status `passed`, verdict `pass`, score `88`. `facebook_schedule_jobs` row `#8` status `done`, `dry_run = 0`, `context_check_id = 37`, no error.
- Current all-eight run counts after refreshing status: `scheduled = 7`, `deleted = 1`, `queued = 229`, `review_needed = 4`. Refreshed `pipeline/weekly_plan.json`; next operational post is Look Good `#181` for `2026-07-01 22:00`, the final first-day page proof.

Follow-up 2026-06-30 standalone Look Good live proof and eight-page day-one completion:
- Look Good `#181`, topic `How to start a skincare routine from scratch`, was approved from `queued` with note `weekly plan look-good one-post live path`.
- Before content generation, Look Good skincare rules were hardened: no clear-skin/anti-aging/acne-cure/treatment-result promises, no guaranteed results for every skin type, use generic product categories, introduce one new product at a time, patch test, stop if irritation occurs, and refer persistent acne/rashes/severe irritation/allergies/skin conditions to a qualified dermatologist or clinician. The local caption guard now flags obvious skincare treatment/result overclaims.
- Higgsfield image generation succeeded using existing session, model `nano-banana`, cost `2` credits, source generation id `271235c8-0a2c-4b88-a963-4bbd433c32c1`, and 100% prompt-matched Asset Library recovery. Initial visual QA showed clean label-free skincare products, but inside a fake social-media UI frame. Claude QA returned `review_needed`, not because of caption safety, but because the visible profile/heart/comment/share UI chrome would look confusing as a Facebook post image.
- The original UI-frame image was backed up in generated assets as `01_how-to-start-a-skincare-routine-from-scratch_ui-frame-original.png`, then the active image file was cropped to the inner product-photo square using PowerShell/.NET image APIs. A first crop attempt used the wrong drawing overload and left a black canvas, but it was safely restored from the backup and re-cropped correctly to `1365x1365`. This avoided spending a second Higgsfield credit.
- Regenerated/retained content passed the local guard. Real QA command `npm.cmd run qa -- --run --id 181` passed after the crop repair with score `91`, verdict `pass`, and moved the post to `qa_done`.
- Look Good page context dry-run verified successfully. Facebook `--prepare` set `scheduled_date = 2026-07-01`, `scheduled_time = 22:00`; `--schedule-dry-run` passed; live command `npm.cmd run facebook -- --live --id 181 --user-data-dir .browser-profile --login-wait-sec 120` re-verified Look Good, uploaded the cropped image, entered caption, set the scheduled time, clicked final Schedule, waited 30s, and confirmed by in-place Content Library row match (`caption+time`).
- DB proof after success: post `#181` status `scheduled`, page `Look Good`, `scheduled_date = 2026-07-01`, `scheduled_time = 22:00`, `scheduled_at = 2026-06-30T23:44:38.632Z`, `quality_verdict = pass`, no error. `image_jobs` latest row `#8` status `done`, `credit_cost = 2`, `source_gen_id = 271235c8-0a2c-4b88-a963-4bbd433c32c1`. `post_quality_checks` latest row `#6` status `passed`, verdict `pass`, score `91`. `facebook_schedule_jobs` row `#9` status `done`, `dry_run = 0`, `context_check_id = 39`, no error.
- First-day eight-page proof is complete. Current all-eight run counts after refreshing status: `scheduled = 8`, `deleted = 1`, `queued = 228`, `review_needed = 4`. Refreshed `pipeline/weekly_plan.json`; next ready posts start the July 2 rotation: Fix It `#92` at 15:00, Cook It `#61` at 16:00, Grow It `#152` at 17:00, Money it `#242` at 18:00, Get Fit `#122` at 19:00, Make It `#212` at 20:00, Tech It `#272` at 21:00, and Look Good `#182` at 22:00.

Follow-up 2026-06-30 standalone weekly batch-run policy:
- User clarified that future batch execution should not pause for human intervention on ordinary repairable QA failures. The desired behavior is auto-repair until the post can pass QA, then schedule, with every attempt recorded in SQLite.
- Image QA failures should auto-regenerate in Higgsfield until they pass when using the unlimited/no-marginal-credit image mode. Do not use a fixed max image retry count in that mode. Examples of repairable image failures: readable text/labels/logos, brand-like visuals, social UI chrome, awkward duplicate props, poor crop, artifacts, or image/caption mismatch.
- Even without a numeric image retry cap, keep operational brakes: stop if Higgsfield session/login breaks, generation repeatedly fails to produce a valid file, the batch hits a wall-clock budget, or repeated QA reasons prove the prompt needs strategy mutation. On repeated image QA failures, mutate the image prompt from QA feedback; if the same failure persists, switch visual strategy entirely, e.g. from product flat lay to unlabeled generic props on a neutral background.
- Caption QA failures should still have a bounded retry budget, because repeated caption failures usually indicate a topic/prompt/schema issue rather than visual randomness. Preferred initial policy: regenerate caption from QA feedback up to 3 times, then skip the candidate and pick the next queued post for that page.
- If QA flags both image and caption, fix image first, then regenerate caption against the final image. Never schedule until the latest QA row is `passed/pass` for the exact image path and caption hash.
- Human intervention should be reserved for non-repairable or external failures: Facebook page context cannot be confirmed, Facebook schedule confirmation fails after submit, Higgsfield login/session cannot be recovered, the batch wall-clock budget is exhausted, or the topic itself is unsafe/stale/high-risk rather than merely needing better image/caption execution.

Follow-up 2026-06-30 standalone weekly batch runner implementation:
- Added `pipeline/batch.js` and `npm.cmd run batch` as the guarded executor for `weekly_plan.json`. The default mode is read-only selection/dry-run. `--execute` runs approve -> image -> content -> QA -> Facebook page-context -> prepare -> schedule dry-run. `--live` adds the final Facebook scheduling step.
- The batch runner deliberately delegates to the existing stage CLIs (`review.js`, `image.js`, `content.js`, `qa.js`, `facebook.js`) instead of duplicating image/content/scheduler logic. This keeps the manual one-post path and batch path on the same code and guardrails.
- Important DB design: `batch.js` opens the standalone SQLite DB only for short reads/writes between child processes via `withDb()`. Do not hold one SQL.js DB handle open while stage child processes mutate/save the same `howto-content.sqlite`, or the parent can overwrite fresher child state on close.
- Batch repair behavior matches the policy above. Image QA failures reset the post to `approved`, mutate `image_prompt` with QA feedback and hard visual requirements, and then regenerate image/content/QA. Caption QA failures reset to `image_done`, preserve repair feedback in `error_message`, and let `content.js` include that feedback in the next Claude prompt. `review_needed` posts with a QA row resume through repair; posts already marked `Batch skipped:` are not auto-approved on rerun.
- Validation proof after implementation: `npm.cmd run check` passed, `npm.cmd run batch -- --day 2026-07-02 --limit 8` selected the next full eight-page rotation without changes, and `npm.cmd run batch -- --day 2026-07-02 --niche fix-it --limit 1` selected only Fix It `#92`. The July 2 ready rotation is Fix It `#92` at 15:00, Cook It `#61` at 16:00, Grow It `#152` at 17:00, Money it `#242` at 18:00, Get Fit `#122` at 19:00, Make It `#212` at 20:00, Tech It `#272` at 21:00, and Look Good `#182` at 22:00.

Follow-up 2026-07-01 standalone July 2 batch rehearsal:
- First non-live `npm.cmd run batch -- --execute --day 2026-07-02 --limit 8 ...` prepared five posts and exposed a repair bug: `content_done` QA failures referenced an undefined `qa` variable before entering repair. Fixed by fetching `latestQa(postId)` before `repairAfterQaFailure()`.
- The Fix It screwdriver candidate `#92` proved a second repair issue: full QA text was being appended into Higgsfield image prompts, including caption/hook feedback. `batch.js` now rebuilds image-repair prompts from the original base image prompt and filters QA down to image-specific reasons/fixes only. It also adds optional `--image-retries N`; default remains unlimited unless a cap is explicitly supplied.
- User set a practical policy for the screwdriver post: after 5 image tries, dump it and choose the next post. `#92` is marked `review_needed` with `Batch skipped: screwdriver visual failed after 5 image repair attempts; choose next Fix It candidate`.
- Regenerated `weekly_plan.json` selected replacements for the failed July 2 slots: Fix It `#93`, Get Fit `#123`, and Tech It `#273`. Running `npm.cmd run batch -- --execute --day 2026-07-02 --limit 8 --image-retries 5 ...` prepared all three successfully.
- `batch.js` now selects both `ready` and `prepared` rows from `weekly_plan.json`. This is required because a non-live `--execute` rehearsal turns rows into `prepared`/`qa_done`; the later `--live` batch must schedule those same prepared rows rather than finding no ready rows or selecting new candidates.
- Current July 2 prepared/QA-passed set before live scheduling: Fix It `#93` 15:00, Cook It `#61` 16:00, Grow It `#152` 17:00, Money it `#242` 18:00, Get Fit `#123` 19:00, Make It `#212` 20:00, Tech It `#273` 21:00, Look Good `#182` 22:00. All have latest QA `passed/pass`; failed candidates `#92`, `#122`, and `#272` remain `review_needed`.

Follow-up 2026-07-01 standalone July 2 live batch proof:
- Live weekly batch succeeded with `npm.cmd run batch -- --live --day 2026-07-02 --limit 8 --image-retries 5 --user-data-dir .browser-profile --facebook-login-wait-sec 120 --higgsfield-login-wait-sec 240`.
- Each page was re-verified before scheduling, each post ran `facebook --prepare` and `facebook --schedule-dry-run`, and each live schedule was confirmed by Facebook Content Library in-place row match using caption+time. No live schedule failures occurred.
- DB proof after the live batch: posts `#93`, `#61`, `#152`, `#242`, `#123`, `#212`, `#273`, and `#182` are all `status = scheduled`, `quality_verdict = pass`, scheduled for `2026-07-02` at 15:00 through 22:00 respectively, with no `error_message`. Latest `facebook_schedule_jobs` rows for those posts are all `status = done`, `dry_run = 0`, context check ids `49`, `51`, `53`, `55`, `57`, `59`, `61`, and `63`.
- Refreshed `weekly_plan.json` after live scheduling: run `20260629222820_d3f0fcc8` now has 16 scheduled posts total (July 1 and July 2 across all eight pages), 217 queued, 7 review_needed, and 1 deleted. Next ready day starts July 3: Fix It `#94` at 15:00, Cook It `#63` at 16:00, Grow It `#153` at 17:00, Money it `#243` at 18:00, Get Fit `#124` at 19:00, Make It `#213` at 20:00, Tech It `#274` at 21:00, and Look Good `#183` at 22:00.

Follow-up 2026-07-01 standalone July 3 live batch proof:
- Non-live batch preparation for July 3 initially passed all eight selected posts: Fix It `#94`, Cook It `#63`, Grow It `#153`, Money it `#243`, Get Fit `#124`, Make It `#213`, Tech It `#274`, and Look Good `#183`. Cook It `#63` required one automatic QA repair, then passed.
- The first July 3 live batch confirmed the first five posts on Facebook: `#94` at 15:00, `#63` at 16:00, `#153` at 17:00, `#243` at 18:00, and `#124` at 19:00. Make It `#213` (`natural body scrub`) failed Facebook schedule confirmation twice after the final submit attempt; no Content Library confirmation row appeared.
- Replacement Make It `#214` (`natural lip balm`) also failed the same Facebook schedule confirmation path. Treat this as an operational caveat for Make It self-care/cosmetic topics: skip them for now and choose a non-cosmetic craft/DIY candidate rather than repeatedly retrying the same Facebook submit.
- Non-cosmetic replacement Make It `#216` (`paint and customise a Jenga set`) prepared, passed QA, scheduled live, and confirmed correctly. Tech It `#274` and Look Good `#183` were then scheduled live and confirmed.
- Final July 3 scheduled set: Fix It `#94` 15:00, Cook It `#63` 16:00, Grow It `#153` 17:00, Money it `#243` 18:00, Get Fit `#124` 19:00, Make It `#216` 20:00, Tech It `#274` 21:00, and Look Good `#183` 22:00. All latest QA rows were `passed/pass`.
- After refreshing `weekly_plan.json`, DB counts were `scheduled = 24`, `queued = 207`, `review_needed = 9`, and `deleted = 1`. Next ready day starts July 4: Fix It `#95` at 15:00, Cook It `#64` at 16:00, Grow It `#154` at 17:00, Money it `#244` at 18:00, Get Fit `#125` at 19:00, Make It `#217` at 20:00, Tech It `#275` at 21:00, and Look Good `#184` at 22:00.

Follow-up 2026-07-01 standalone July 4 live batch proof:
- Non-live batch preparation for July 4 prepared seven of the initial eight posts: Fix It `#95`, Cook It `#64`, Grow It `#154`, Money it `#244`, Get Fit `#125`, Make It `#217`, and Look Good `#184`. Tech It `#275` (`How to spot when AI is giving you wrong information`) was skipped after the content retry budget was exhausted because generated captions repeatedly failed the local guard for fewer than 3 numbered steps.
- Refreshed `weekly_plan.json` selected replacement Tech It `#276` (`How to use AI to boost your daily productivity`) for the July 4 21:00 slot. `#276` prepared cleanly: Higgsfield session alive, image generated/recovered at 100% prompt similarity, content generated, QA passed, Tech It page context verified, and schedule dry-run passed.
- Live weekly batch succeeded with `npm.cmd run batch -- --live --day 2026-07-04 --limit 8 --image-retries 5 --user-data-dir .browser-profile --facebook-login-wait-sec 120 --higgsfield-login-wait-sec 240`. Each page context was re-verified before live submit. Facebook confirmation used calendar caption+time+date matches where available, with Content Library row fallback for Make It `#217` and Look Good `#184`.
- Final July 4 scheduled set: Fix It `#95` 15:00, Cook It `#64` 16:00, Grow It `#154` 17:00, Money it `#244` 18:00, Get Fit `#125` 19:00, Make It `#217` 20:00, Tech It `#276` 21:00, and Look Good `#184` 22:00. All have `status = scheduled`, `quality_verdict = pass`, and no post `error_message`.
- DB proof after refresh: total post counts are `scheduled = 32`, `queued = 258`, `review_needed = 10`, and `deleted = 1`. `#275` remains `review_needed` with `Batch skipped: content retry budget exhausted: Generated caption failed quality guard: caption has fewer than 3 numbered steps`.
- Refreshed `weekly_plan.json` after the live proof. Next ready day starts July 5: Fix It `#96` at 15:00, Cook It `#65` at 16:00, Grow It `#155` at 17:00, Money it `#245` at 18:00, Get Fit `#126` at 19:00, Make It `#218` at 20:00, Tech It `#277` at 21:00, and Look Good `#185` at 22:00.

Follow-up 2026-07-01 standalone batch timing tracking:
- Added DB-backed timing for future how-to batch invocations. `batch.js` now creates a `batch_runs` row for every dry-run, execute, and live batch with mode, selected/result counts, start/end timestamps, `duration_ms`, result JSON, and final status.
- Child stage calls from `batch.js` now emit `batch_stage_complete` / `batch_stage_failed` events with `batchRunId`, label, args, exit code, and `duration_ms`, allowing later breakdowns of image/content/QA/Facebook time.
- Added `npm.cmd run timings` to inspect recent batch durations, plus `npm.cmd run timings -- --id N --stages` for per-stage totals. Validation proof: `npm.cmd run check` passed, `npm.cmd run batch -- --day 2026-07-05 --limit 1` created `batch_runs` id `1`, and `npm.cmd run timings -- --id 1 --stages` reported the dry-run duration with no child stages, as expected.
- Timing starts from this implementation forward. Earlier July 2-4 live batches remain documented in AGENTS.md but were not backfilled into `batch_runs`.

Follow-up 2026-07-01 standalone July 5 image/FB overlay hardening and live proof:
- Human review caught a Cook It image QA miss: post `#65` Bolognese had passed QA while showing the food image inside a fake phone/social-post frame with profile/like/comment UI. Treat this as a production-blocking visual pattern. `pipeline/image.js` now adds explicit prompt/negative-prompt bans for fake social frames, profile icons, like/comment/share UI, phone/tablet screen mockups, and device bezels framing the subject. `pipeline/qa.js` now hard-fails fake social/app UI wrappers and requires standalone editorial/photo-style images.
- Monitoring a running batch with DB reads exposed a recovery hazard: `openPipelineDb()` crash recovery could reset active `image_generating`/`scheduling`/QA rows if a second process opened the DB mid-run. `pipeline/db.js` now only resets running rows after a stale threshold of 2 hours. Do not lower this casually; it protects live batch runs from Codex monitoring side effects.
- July 5 non-live preparation required repair. Batch `#3` (`--execute --day 2026-07-05 --limit 8 --image-retries 5`) ended `needs_attention` after 39m20s with 7 prepared and Money `#245` skipped for caption retry exhaustion (`fewer than 3 numbered steps`). Replacement Money `#246` prepared successfully in batch `#4` (3m41s). Dry selector batch `#5` confirmed the final set.
- July 5 live scheduling used the final set: Fix It `#96` 15:00, Cook It `#65` 16:00, Grow It `#155` 17:00, Money it `#246` 18:00, Get Fit `#126` 19:00, Make It `#218` 20:00, Tech It `#277` 21:00, and Look Good `#185` 22:00.
- First live batch `#6` scheduled the first five posts but failed on Make It `#218` after final submit because Facebook did not produce a matching confirmation row before timeout. A read-only diagnostic showed the visible scheduled rows were older Make It posts, not the `#218` chess-set caption, so retry was safe. Batch `#7` retried Make It but hit the same old-code confirmation failure.
- Facebook can show a modal after schedule submit: `Make it easier to contact you` / `Add WhatsApp button` with a `Not now` button. This overlay can block or confuse confirmation. `src/main/social/social-facebook-uploader.js` now detects this dialog and clicks `Not now` or closes it; popup dismissal is called before/after Schedule-for-later, before final Schedule, and during the submit-settle loop. The patched one-post command `npm.cmd run facebook -- --live --id 218 --user-data-dir .browser-profile --login-wait-sec 120` successfully scheduled Make It `#218`; log proof included `Dismissed WhatsApp button prompt`.
- Facebook Professional Dashboard Calendar day URLs can also return `This content isn't available right now` for `professional_dashboard/content_calendar/?time_offset=...&view=DAY`. This is not proof that the scheduled post failed. `src/main/social/social-facebook-uploader.js` and `src/main/shorts/facebook-uploader.js` now detect that unavailable-content surface and immediately fall back to Scheduled tab row confirmation instead of spinning on calendar candidates. Look Good `#185` live proof hit this exact page, logged `calendar day view unavailable; falling back to Content Library confirmation`, then confirmed by in-place Scheduled row match.
- Final live batch `#8` completed in 9m19s and scheduled the two remaining posts (`#277`, `#185`). DB proof after verification: all eight July 5 posts are `status = scheduled`, `quality_verdict = pass`, no `error_message`; Money `#245` remains `review_needed` and was replaced by `#246`. Timing proof: `npm.cmd run timings -- --limit 6 --stages` shows batch `#8 live done`, `selected=8`, `scheduled=8`, `failed=0`, with Facebook live time 8m17s over 2 calls.

Follow-up 2026-07-01 standalone visual prompt dedup guard:
- Existing how-to duplicate protection only fingerprinted topic/hook content, so it could stop repeated post ideas but not repeated image compositions. Repeated visual recipes such as overhead hands on a workbench, bowls on a table, phone/mock social frames, or generic desk/laptop scenes could still recur across different topics.
- Added `pipeline/visual-dedup.js` and `pipelineConfig.visualDedup`. The guard fingerprints each image prompt by composition, setting, actor framing, prop family, and visual tokens, then compares against recent scheduled/generated prompts. Defaults: same-niche threshold `0.64`, global threshold `0.74`, recent limit `80`, statuses `scheduled/scheduling/qa_done/content_done/image_done/image_generating`.
- `pipeline/image.js` now runs visual dedup before dry-run/live Higgsfield payload creation. Live mode persists the varied prompt plus `posts.visual_fingerprint`, `posts.visual_dedupe_reason`, and `posts.visual_dedupe_checked_at`; `image_jobs` records `visual_fingerprint` and `visual_dedupe_json`; a `visual_dedup_prompt_varied` event records the matched prior post and strategy. Dry-run mode records the decision in the image job/manifest without rewriting the post prompt.
- Positive proof: a synthetic reuse of scheduled Fix It `#96` was detected as 84% similar (`same overhead angle`, `same bathroom setting`, overlapping prompt tokens) and automatically received a wider environmental-shot variation. A dry-run for queued Fix It `#97` wrote visual fingerprint metadata while leaving the post queued and unchanged.

Follow-up 2026-07-11 YouTube Shorts + engagement reuse implementation plan:
- User wants to explore reusing the existing Shorts and Engagement modules for YouTube. Treat this as a plan only; no YouTube Shorts uploader or multi-platform publish-job schema has been implemented yet.
- Target YouTube Studio channel dashboard provided by user: `https://studio.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA`. Use this as the intended Playwright/browser target for future YouTube Studio dry-runs after login/session handling is added.
- Best reuse base is the Electron Shorts/Social stack, not the standalone `pipeline/` Facebook how-to module: `src/main/shorts/shorts-scheduler.js` already handles clip selection, FFmpeg assembly, short calendar rows, and short metadata; `src/main/social/social-planner.js` already plans character intros, pre-short teasers, and post-short recaps around scheduled shorts; `src/main/social/social-copy-generator.js` already generates Nollywood fan-page engagement copy from script/short context.
- Current platform coupling to preserve carefully: `src/main/shorts/facebook-uploader.js` and `src/main/social/social-facebook-uploader.js` are Facebook-only Playwright automations; `shorts.status = scheduled` currently means scheduled on Facebook; `shorts.facebook_post_id` and `social_posts.facebook_post_id` are first-class persistence fields; `ShortsScheduler.generateSEO()` currently prompts as a Facebook Reels specialist and includes Facebook-specific hashtags such as `#fbreels`.
- Recommended architecture: extract a platform boundary rather than renaming or rewriting the existing Facebook path. Shape: `ShortsController -> ShortsScheduler` for platform-neutral planning/assembly, `ShortMetadataGenerator` or prompt profiles for `facebook` vs `youtube`, and `ShortPublisherAdapter` implementations for `facebook` and `youtube`.
- Recommended DB design is additive: add a `short_publish_jobs` table instead of overloading the existing `shorts` row. Suggested fields: `id`, `short_id`, `platform`, `status`, `scheduled_date`, `scheduled_time`, `title`, `description`, `hashtags_json`, `remote_post_id`, `remote_url`, `upload_confirmed_at`, `error_message`, `proof_json`, timestamps, and a unique key on `(short_id, platform)` or `(short_id, platform, scheduled_date)` depending on desired repost semantics. Add an equivalent platform-specific publish table for `social_posts` only if YouTube Community posts become part of scope.
- Implementation order: (1) add adapter interface and wrap existing Facebook short/social uploaders with no behavior change; (2) add publish-job schema and helper methods while keeping legacy Facebook fields intact for compatibility; (3) add YouTube Shorts metadata prompt/profile using `src/main/publish/seoGenerator.js` as style guidance but tuned for 9:16 shorts, concise title, description, hashtags, and scheduled visibility; (4) add a dry-run `YouTubeShortsUploader` that launches YouTube Studio, verifies login/channel context, and validates fields without publishing; (5) only after dry-run proof, add one-short live upload/schedule with confirmation proof and recovery.
- Safety gates for YouTube should mirror Facebook/Higgsfield operational posture: one-short live gate first, persistent browser profile support, explicit channel-context proof before any upload, file existence and duration/aspect checks, metadata sanity checks, no broad batch upload until one live short is proven, and DB proof before marking any job scheduled/published.

Follow-up 2026-07-11 YouTube Shorts + engagement reuse plan revision:
- Re-examination confirmed the plan should be more conservative than a selector swap. Do not treat YouTube as a Facebook uploader with different Playwright clicks; keep short assembly and engagement planning reusable, but move platform publishing state behind an adapter boundary.
- Add `short_publish_jobs` as the durable multi-platform state layer. Keep legacy `shorts.status = scheduled`, `shorts.facebook_post_id`, and `social_posts.facebook_post_id` intact so current Facebook behavior remains compatible. YouTube proof should live in the job row until the UI and reporting layers are deliberately migrated.
- YouTube Shorts profile rules should be explicit: square or vertical video, hard maximum 3 minutes, title maximum 100 chars, description maximum 5000 chars, channel-context proof for `https://studio.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA`, and default AI altered/generated disclosure for this photorealistic AI drama pipeline unless the user deliberately overrides it.
- Initial YouTube live proof should prefer <=60s shorts even though YouTube supports up to 3 minutes, because Shorts over 1 minute have extra Content ID blocking risk if a claim appears. Longer 60-180s YouTube Shorts can be enabled later after one-short proof and content-rights confidence.
- Engagement reuse should not auto-map Facebook Page posts to YouTube. First phase may reuse engagement context for Shorts descriptions, titles, pinned-comment ideas, or future Community-post copy, but YouTube Community publishing is a separate scope requiring its own eligibility, UI, and confirmation proof.
- Implementation should proceed in this order: (1) add adapter/profile/schema foundation and route Facebook through the Facebook adapter with no behavior change; (2) add YouTube dry-run validation only; (3) prove YouTube Studio channel context without uploading; (4) only with explicit user approval, run a one-short private/unlisted/scheduled live proof and require DB/video proof before any batch.

Follow-up 2026-07-11 YouTube Studio dry-run channel proof:
- Implemented the no-upload YouTube Studio channel-context verifier for Shorts. The verifier lives in `src/main/shorts/youtube-studio-uploader.js`, is exposed through `YouTubeShortPublisherAdapter.verifyChannelContext()`, and only navigates/reads page state. It must not click Create, set a file input, or open the upload wizard during channel proof.
- The verifier uses the target dashboard `https://studio.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA`, extracts expected channel id `UCObQBiWc7kI4Q1PPpQZiuxA`, and requires all proof before success: Studio host loaded, expected channel id present in URL/body, Studio dashboard indicators present, and no Google/YouTube login blocker.
- First real dry-run with `.browser-profile-youtube` reached Google Security Checkup and failed safely with `YOUTUBE_CHANNEL_CONTEXT_NOT_VERIFIED`; no upload controls were touched. After user completed login, rerunning in a fresh tab in the same persistent profile succeeded.
- Important implementation detail: YouTube Studio verifier now opens a new page/tab by default inside the persistent context (`newPageOnLaunch !== false`). This avoids reusing a stale Security Checkup/login tab from the same profile.
- Live no-upload proof succeeded on 2026-07-11: final URL `https://studio.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA`, title `Channel dashboard - YouTube Studio`, expected channel id matched, `studioLoaded = true`, `loginRequired = false`, indicators included `studioHost = true`, `ytcpAppCount = 1`, `createButtonCount = 3`, and `dashboardText = true`.
- Current state remains dry-run only. `YouTubeShortPublisherAdapter.scheduleShort()` still returns `YOUTUBE_DRY_RUN_ONLY` even after channel proof. Next step should be metadata/job preparation and an explicit user-approved one-short private/unlisted/scheduled upload proof; do not batch upload.

Follow-up 2026-07-11 YouTube Shorts job-prep implementation:
- Implemented YouTube Shorts metadata/job preparation without upload. New module `src/main/shorts/youtube-job-prep.js` builds YouTube-shaped title/description/hashtags from existing short metadata, removes Facebook-only tags such as `#fbreels`, defaults upload settings to `visibility = private`, `madeForKids = false`, and `aiDisclosure = true`, and validates file/duration/aspect gates.
- Added `metadata_json` and `validation_json` to `short_publish_jobs` via migration `025-short-publish-job-metadata.sql`; migration `024-short-publish-jobs.sql` also includes the columns for fresh databases. Keep `proof_json` reserved for real Studio upload/schedule proof.
- Added DB helper support for `metadata_json` / `validation_json`. A prepared YouTube job is marked `ready` only when video exists, duration/dimensions are verified, aspect is square/vertical, title/description fit YouTube limits, AI disclosure decision is present, and first-proof duration is <=60s by default. Otherwise the job is persisted as `blocked` with validation errors.
- Exposed no-upload controller methods: `ShortsController.prepareYouTubePublishJob(shortId, options)` and `ShortsController.prepareNextYouTubePublishJob(projectId, options)`. These create/update `short_publish_jobs` rows only; they do not launch YouTube Studio or touch upload controls.
- Validation tests added in `test/test-shorts-youtube-job-prep.js`: ready vertical <=60s job, removal of Facebook-only hashtags, blocked first-proof >60s job, blocked landscape video, and blocked missing video. The implementation was tested with mocked DB writes only; no live/app DB publish job was created yet.

Follow-up 2026-07-11 YouTube Shorts real DB publish-job prep:
- Refreshed the YouTube job-prep implementation after real DB readback. YouTube jobs now default to `visibility = private` with no inherited Facebook `scheduled_date` / `scheduled_time`; scheduled YouTube visibility is blocked unless `options.scheduledDate` is explicit. Description cleanup strips all prior hashtag lines and appends one normalized YouTube hashtag line, excluding Facebook-only tags including `#fbreels` and `#reels`.
- Real AppData DB path used: `C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite`. A backup was created before the refresh at `C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite.before-youtube-short-job-prep-2026-07-11T20-13-13-314Z.bak`.
- Applied migrations `024-short-publish-jobs.sql` and `025-short-publish-job-metadata.sql` to the real app DB without running full app startup recovery. Prepared/refreshed one no-upload row: `short_publish_jobs.id = 1`, `short_id = 31`, `platform = youtube_shorts`, `status = ready`, `scheduled_date = null`, `scheduled_time = null`, private visibility, `aiDisclosure = true`, `madeForKids = false`, `ageRestricted = false`, `paidPromotion = false`.
- The prepared source short is `2026-05-08_55658c96` short 1, file `C:\Users\chris\Cowork_Nollywood\Mass-produced\Mass production - Nollywood\2026-05-08_55658c96\shorts\short_001.mp4`; ffprobe proof showed the file exists, `durationSeconds = 30.083333`, `width = 1080`, `height = 1920`. Validation JSON is ok with no errors/warnings.
- Readback proof after refresh: no YouTube upload or schedule proof exists (`remote_post_id`, `remote_url`, `upload_confirmed_at`, and `proof_json` are all null). The current YouTube adapter remains dry-run only; do not mark anything scheduled/published until a future explicit one-short Studio upload proof succeeds.
- Validation commands passed after the refresh: `test-shorts-youtube-job-prep`, `test-shorts-youtube-publish-foundation`, `test-shorts-seo-retry-upload-guard`, `test-shorts-facebook-confirmation`, and `test-social-schedule-window`.
- Next implementation step: build a YouTube Studio form dry-run/upload-wizard inspector for the prepared job, still stopping before file selection or publish/schedule unless the user explicitly approves a one-short private/unlisted live proof.
Follow-up 2026-07-11 YouTube Studio upload-entry dry-run proof:
- Added a no-upload upload-wizard inspector. `YouTubeStudioUploader.inspectUploadWizard()` verifies channel context, reads the Create/upload entry state, and optionally can open the upload dialog for inspection only; it never sets a file input and returns `noFileSelected = true`. `YouTubeShortPublisherAdapter.inspectUploadWizard()` exposes this separately from `scheduleShort()`, which still returns `YOUTUBE_DRY_RUN_ONLY`.
- Added `ShortsController.inspectYouTubeUploadWizard(jobId, options)`. It requires an existing `short_publish_jobs` row with `platform = youtube_shorts`; by default it only checks channel context and upload entry proof. Passing `openWizard: true` is a separate dry-run step and must still stop before file selection.
- Mocked validation passed in `test-shorts-youtube-publish-foundation.js`, including a no-file upload-entry inspection and adapter delegation. The full focused suite still passed: YouTube foundation, YouTube job prep, Shorts SEO retry upload guard, Shorts Facebook confirmation, and social schedule window.
- Live no-click proof with `.browser-profile-youtube` succeeded on 2026-07-11 using `openWizard = false`: final URL `https://studio.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA`, title `Channel dashboard - YouTube Studio`, `channelIdMatched = true`, `studioLoaded = true`, `loginRequired = false`, `noFileSelected = true`, `hasCreateEntry = true`, `hasUploadVideosEntry = true`, `createButtonCount = 3`, `uploadVideosButtonCount = 1`, and `wizardProof = null` because the upload dialog was not opened.
- Next step, only if explicitly approved, is an `openWizard: true` dry-run to click Create -> Upload videos, verify the upload dialog/file input/select-files controls, then close it without selecting a file. Do not run a real one-short upload until that dialog proof is clean and the user approves a private/unlisted live proof.
Follow-up 2026-07-11 YouTube Studio upload-dialog dry-run proof:
- Ran the explicitly approved `openWizard = true` dry-run with `.browser-profile-youtube`. The automation verified the target channel dashboard, clicked only `Create -> Upload videos`, inspected the upload dialog, then closed the browser without selecting a file.
- Proof from the dry-run: final URL `https://studio.youtube.com/channel/UCObQBiWc7kI4Q1PPpQZiuxA`, title `Channel dashboard - YouTube Studio`, `channelIdMatched = true`, `studioLoaded = true`, `loginRequired = false`, `hasCreateEntry = true`, `hasUploadVideosEntry = true`, `createButtonCount = 3`, `uploadVideosButtonCount = 1`.
- Upload dialog proof: `uploadDialogVisible = true`, `uploadDialogCount = 1`, `fileInputCount = 1`, `selectFilesButtonCount = 1`, `uploadVideosText = true`, `selectFilesText = true`, and `noFileSelected = true`. No file path was set, no video was uploaded, and no publish/schedule action occurred.
- The next gate is no longer upload-dialog discovery; it is implementing a one-short live-proof uploader path that can set the prepared job's video file and metadata, select private/unlisted visibility, stop before final publish/schedule unless explicitly approved, and only mark DB proof after YouTube confirms the uploaded draft/scheduled item. Keep `YouTubeShortPublisherAdapter.scheduleShort()` dry-run-only until that path is implemented and separately approved.
Follow-up 2026-07-11 YouTube Shorts one-short live schedule proof:
- User explicitly approved proceeding with e2e YouTube scheduling and said they would cancel it later. Implemented a guarded live proof path: `YouTubeStudioUploader.uploadAndScheduleShort()`, `YouTubeShortPublisherAdapter.uploadAndScheduleShort()`, `ShortsController.scheduleYouTubePublishJob()`, and temp runner `tmp_youtube_schedule_job.mjs` for AppData-safe live proof without broad app startup recovery.
- First live schedule attempt for job `1` reached the YouTube Visibility/Schedule screen but failed before final confirmation because the generic date selector targeted the time input, leaving `7/18/2026` in the time field with Studio showing `Invalid Time`. User screenshot confirmed the date picker is a separate control left of the time field. No DB upload proof was written for that failed attempt.
- Scheduler fix: moved `ytcp-datetime-picker input` to the time selector only, skipped date entry by default, used Studio's visible default date for this proof, filled the time field as `6:00 PM`, pressed Enter/Tab to commit dropdown input, and added an `Invalid Time` pre-final-click guard plus visible date/time readback proof.
- Successful live e2e proof then ran for `short_publish_jobs.id = 1`, `short_id = 31`, scheduling for `2026-07-12 18:00`. Studio channel context matched `UCObQBiWc7kI4Q1PPpQZiuxA`; upload dialog, file input, metadata fill, made-for-kids = No, Visibility/Schedule step, and final Schedule click completed.
- DB proof after success: job `1` status `scheduled`, `scheduled_date = 2026-07-12`, `scheduled_time = 18:00`, `remote_post_id = PvmGLH0nw-8`, `remote_url = https://studio.youtube.com/video/PvmGLH0nw-8/analytics/tab-overview/period-default`, `upload_confirmed_at = 2026-07-11T23:31:02.094Z`, `has_upload_proof = true`. AppData backup before successful attempt: `C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite.before-youtube-short-live-schedule-2026-07-11T23-29-19-029Z.bak`.
- Proof details included visible schedule date `Jul 12, 2026`, time value changed from `12:00 AM` to `6:00 PM`, `invalidTimeVisible = false`, final schedule click `clicked = true`, confirmation inferred from Studio video URL/video id `PvmGLH0nw-8`.
- Caveat: failed pre-success attempts may have left autosaved YouTube Studio draft uploads with the same title. Do not delete them without explicit user approval; they are not DB-confirmed scheduled posts. If cleanup is requested, inspect Studio Content/Drafts for duplicates of `She Knows What You're Hiding 👀` and delete only unconfirmed drafts, leaving video id `PvmGLH0nw-8` intact unless the user asks to cancel it too.
- Validation after the successful proof passed: `test-shorts-youtube-publish-foundation`, `test-shorts-youtube-job-prep`, `test-shorts-seo-retry-upload-guard`, `test-shorts-facebook-confirmation`, and `test-social-schedule-window`.
Follow-up 2026-07-11/12 YouTube Shorts cleanup and proof correction:
- Important correction to the prior live-proof note: post-run inspection proved `PvmGLH0nw-8` is the existing long-form video `She Chose Wealth Over Blood — Abuja Changed Her | Nigerian AI Film`, not the new short. The schedule confirmation logic had incorrectly accepted an unrelated Studio dashboard/Top Content link after the failed schedule flow. Treat the previous DB `scheduled` state as invalid.
- Inspected YouTube Studio Shorts tab (`/videos/short`) for `She Knows What You're Hiding`. It showed exactly three matching `0:31` Shorts rows, all `Draft` / `Edit draft`, and no scheduled/published short. Row controls proved per-row Options menu exposes `Delete forever`.
- User had asked to proceed with cleanup. Deleted all three matching draft rows via row Options -> `Delete forever`; verification after cleanup showed `bodyHasTitle = false`, `matchedRows = []`, `matchedLinks = []`, and Shorts tab body `No content available` for the matching title.
- Repaired AppData DB job `1`: `status = upload_failed`, `scheduled_date = null`, `scheduled_time = null`, `remote_post_id = null`, `remote_url = null`, `upload_confirmed_at = null`, `proof_json = null`, `has_upload_proof = false`, and `error_message = Cleaned 3 duplicate YouTube Shorts drafts; previous schedule proof was invalid and matched unrelated long-form video PvmGLH0nw-8. No active YouTube Shorts upload proof remains.` Backup before repair: `C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite.before-youtube-job1-proof-repair-2026-07-12T03-01-14-197Z.bak`.
- Hardened schedule confirmation so it no longer scans arbitrary page anchors. Future confirmation must come from explicit schedule confirmation text and must not accept a draft-list page or unrelated dashboard link. Next implementation work before retrying live scheduling: resume/reuse an existing draft when present, prevent duplicate uploads on retry, and add robust date-picker selection instead of relying on Studio's default date.
- Validation after cleanup/proof repair passed: `test-shorts-youtube-publish-foundation`, `test-shorts-youtube-job-prep`, `test-shorts-seo-retry-upload-guard`, `test-shorts-facebook-confirmation`, and `test-social-schedule-window`.
Follow-up 2026-07-11/12 YouTube Shorts duplicate-safe resumed schedule proof:
- After cleanup/proof correction, implemented duplicate-safe retry behavior. `uploadShortDraft()` now checks the YouTube Studio Shorts tab for matching Draft rows before uploading. Default behavior fails closed with `YOUTUBE_DUPLICATE_DRAFTS_FOUND`; `existingDraftPolicy = resume` resumes a matching draft instead of uploading a duplicate.
- Hardened final schedule verification: after final Schedule click, the automation waits on the upload surface before navigating away, then verifies the intended title on the Shorts content tab. DB proof is written only when a matching row is non-draft and scheduled/public/unlisted with a concrete Studio/thumbnail video id. This prevents unrelated dashboard links from being accepted as proof.
- Added guarded date handling: date picker is validated by visible date, same-month date selection is supported, time field is targeted separately, Enter/Tab commits the time value, and `Invalid Time` blocks final Schedule.
- Retry flow after the failed duplicate-safe upload found one matching Draft row (`thumbnailIds = [uQGmdxd0TeM]`), resumed it (`resumedExistingDraft = true`, `fileSelected = false`), refilled title/description, selected `madeForKids = No`, moved through Visibility/Schedule, set date/time to `Jul 12, 2026` / `6:00 PM`, clicked Schedule, waited before Shorts-tab verification, then confirmed the Shorts row was non-draft `Scheduled`.
- Correct final YouTube proof: `short_publish_jobs.id = 1`, `short_id = 31`, `status = scheduled`, `scheduled_date = 2026-07-12`, `scheduled_time = 18:00`, `remote_post_id = uQGmdxd0TeM`, `remote_url = https://studio.youtube.com/video/uQGmdxd0TeM/edit`, `upload_confirmed_at = 2026-07-12T03:18:17.082Z`, `has_upload_proof = true`. The valid scheduled Shorts row text included `0:31`, `She Knows What You're Hiding 👀`, `Scheduled`, `Jul 12, 2026`, `Scheduled`, `0`, `0`.
- The old unrelated id `PvmGLH0nw-8` remains invalid for the short and should not be used as proof. Current valid short id is `uQGmdxd0TeM`. User previously said they would cancel later; if asked to cancel/delete, target `uQGmdxd0TeM` only.
- Validation after the resumed proof passed: `test-shorts-youtube-publish-foundation`, `test-shorts-youtube-job-prep`, `test-shorts-seo-retry-upload-guard`, `test-shorts-facebook-confirmation`, and `test-social-schedule-window`.
Follow-up 2026-07-12 YouTube Shorts productized scheduling path:
- Productized the verified YouTube Shorts proof path beyond tmp_youtube_schedule_job.mjs. Main process IPC now exposes prepareYouTubePublishJob, prepareNextYouTubePublishJob, inspectYouTubeUploadWizard, scheduleYouTubePublishJob, and scheduleNextYouTubePublishJob under the shorts namespace; preload mirrors these as window.api methods for renderer use.
- ShortsController.scheduleYouTubePublishJob now carries the live-run safety defaults: confirmSchedule=true is still required, persistent browser profile falls back to controller userDataDir, existingDraftPolicy defaults to resume, setStudioDate defaults true, verifyInContentTab defaults true, and contentVerifyDelayMs defaults 45000ms before navigating to the Shorts tab for proof.
- getStatus(projectId) now returns youtubeJobs from short_publish_jobs so UI or tools can inspect prepared/scheduled YouTube rows without touching legacy Facebook status.
- YouTubeStudioUploader guardrails were tightened: resume mode only resumes exactly one matching draft and fails ambiguous duplicates, upload-surface confirmation text is not sufficient unless explicitly overridden, final proof remains the Shorts content row, invalid time still blocks the final Schedule click, and youtube.com/shorts/<id> URLs now parse video ids directly.
- Added test/test-shorts-youtube-uploader-guards.js for duplicate draft fail-closed behavior, single-draft resume, ambiguous duplicate rejection, invalid-time blocking, and false upload-surface confirmation rejection. Focused validation passed: test-shorts-youtube-uploader-guards, test-shorts-youtube-publish-foundation, test-shorts-youtube-job-prep, test-shorts-seo-retry-upload-guard, test-shorts-facebook-confirmation, and test-social-schedule-window. Node --check also passed for src/main/shorts/index.js, src/main/shorts/youtube-studio-uploader.js, src/main/main.js, src/preload/preload.js, and the new guard test.
- The valid scheduled YouTube Short from the live proof remains uQGmdxd0TeM scheduled for 2026-07-12 18:00. The old PvmGLH0nw-8 id remains invalid for this short and must not be used as proof. If cancellation or deletion is requested, target uQGmdxd0TeM only.

Follow-up 2026-07-12 YouTube Shorts renderer UI wiring:
- Added YouTube Shorts controls to the Shorts tab under the existing Facebook upload section. The tab title now reflects Facebook Reels plus YouTube Shorts. The UI includes Prepare Next, Inspect Wizard, schedule date/time inputs, Schedule Short, status text, and a YouTube publish-job proof table.
- Renderer state now stores youtubeJobs returned by ShortsController.getStatus(projectId). The job table shows job id, short number, scheduled date/time, status, and remote proof id/link when present. Button enablement is status-aware: inspect uses ready/planned/blocked/upload_failed jobs; schedule uses ready/draft_uploaded/upload_failed jobs and requires explicit date/time.
- The Schedule Short button keeps a browser confirm dialog before any live YouTube Studio action, then calls window.api.scheduleYouTubeShortPublishJob(job.id, { confirmSchedule: true, existingDraftPolicy: resume, setStudioDate: true, verifyInContentTab: true }). It does not auto-schedule merely by selecting a project or preparing a job.
- Prepare Next calls window.api.prepareNextYouTubeShortPublishJob(projectId). Inspect Wizard calls window.api.inspectYouTubeUploadWizard(job.id, { openWizard: true, closeWizard: true }) and remains a no-file-selected dry run. Existing Facebook upload controls remain unchanged.
- Added test/test-shorts-youtube-renderer-ui.js to assert the renderer controls, handlers, preload APIs, main IPC channels, explicit confirmation, and inline script parse. Validation passed: test-shorts-youtube-renderer-ui, test-shorts-youtube-uploader-guards, test-shorts-youtube-publish-foundation, test-shorts-youtube-job-prep, test-shorts-seo-retry-upload-guard, test-shorts-facebook-confirmation, and test-social-schedule-window. Node --check passed for the new renderer UI test.
- Valid scheduled live proof remains YouTube Short uQGmdxd0TeM for 2026-07-12 18:00. If cancellation/deletion is requested, target uQGmdxd0TeM only; PvmGLH0nw-8 is unrelated long-form proof and remains invalid for this short.

Follow-up 2026-07-12 YouTube Shorts Electron UI smoke proof:
- Ran a non-production Electron smoke test with APPDATA/LOCALAPPDATA redirected to .tmp-youtube-ui-smoke and a copied AppData SQLite DB. The real production AppData DB was not launched against or modified by this UI smoke.
- Smoke runner scripts/youtube-ui-smoke.mjs launched Electron, opened the Shorts tab, selected project 2026-05-08_55658c96, and verified the YouTube Shorts section rendered with existing job proof: job 1, short_id 31, short_number 1, status scheduled, scheduled_date 2026-07-12, scheduled_time 18:00, remote_post_id uQGmdxd0TeM, remote_url https://studio.youtube.com/video/uQGmdxd0TeM/edit.
- Initial UI state was correct for an already scheduled job: Prepare Next enabled; Inspect Wizard disabled with title Prepare a YouTube job first; Schedule Short disabled with title Prepare a schedulable YouTube job first.
- The smoke clicked Prepare Next in the copied DB only. It created temp-only YouTube job #3 with status ready, increased copied-profile youtubeJobs from 1 to 2, and enabled Schedule Short for job #3. This does not affect the real AppData DB.
- The smoke overrode window.confirm to return false and wrapped window.api.scheduleYouTubeShortPublishJob to detect accidental calls. Clicking Schedule Short set confirmCalled = true and scheduleApiCalledAfterConfirmFalse = false, proving the renderer confirm gate blocks the live schedule API when cancelled.
- Screenshot artifact is written to .tmp-youtube-ui-smoke/youtube-ui-smoke.png. The copied APPDATA folder .tmp-youtube-ui-smoke/ is now ignored in .gitignore to avoid staging copied DB/store data. No new live YouTube action was taken; valid real scheduled Short remains uQGmdxd0TeM and PvmGLH0nw-8 remains invalid for this short.

Follow-up 2026-07-12 YouTube Shorts smoke-runner packaging and DB repair:
- Moved the Electron UI smoke runner from tmp_youtube_ui_smoke.mjs to scripts/youtube-ui-smoke.mjs. It now copies the source AppData DB into .tmp-youtube-ui-smoke/AppData/Roaming/nollywood-ai-pipeline/nollywood-pipeline.sqlite automatically and writes screenshots to .tmp-youtube-ui-smoke/youtube-ui-smoke.png.
- Added NOLLYWOOD_USER_DATA_DIR support in src/main/main.js before electron-store is constructed. The smoke runner sets this to the ignored copied-profile appUserData directory so Electron app.getPath(userData), electron-store, and the DB path all resolve to the copied profile. This is safer than relying on APPDATA/LOCALAPPDATA alone on Windows.
- A packaged smoke run before the override exposed unintended real DB rows: short_publish_jobs ids 3 and 4, short_ids 32 and 33, status ready, no remote proof. Repaired the real AppData DB with a backup at C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite.before-youtube-ui-smoke-repair-2026-07-12T21-12-12-428Z.bak, deleting only ids 3 and 4 with platform youtube_shorts, status ready, and null remote proof. The real DB now contains only job 1 for YouTube Shorts.
- Reran scripts/youtube-ui-smoke.mjs after the NOLLYWOOD_USER_DATA_DIR fix. It copied the real DB, saw jobCountBefore = 1 with valid scheduled proof uQGmdxd0TeM, created temp-only copied-profile job #5, verified confirmCalled = true and scheduleApiCalledAfterConfirmFalse = false, and left the real AppData DB unchanged. A direct read-only sql.js verification after the smoke showed only job 1 remains in the real DB.


Follow-up 2026-07-12 YouTube Shorts test upload deletion:
User requested deletion of the live e2e test upload after commit. Targeted only YouTube Short uQGmdxd0TeM on channel UCObQBiWc7kI4Q1PPpQZiuxA; the unrelated long-form PvmGLH0nw-8 remains invalid as Shorts proof and was not touched.
Pre-delete Studio Shorts tab proof found exactly one matching row for uQGmdxd0TeM: title She Knows What You're Hiding, duration 0:31, visibility/status Scheduled, date Jul 12, 2026, and links/thumbnails pointing to https://studio.youtube.com/video/uQGmdxd0TeM/edit.
Deleted the row through its three-dot menu -> Delete forever, checked the permanent-delete confirmation box, and confirmed deletion. Post-delete verification on the Shorts tab showed no matching rows and body text No content available.
Repaired AppData DB job 1 after deletion. Final readback: short_publish_jobs.id = 1, platform = youtube_shorts, status = upload_failed, scheduled_date = null, scheduled_time = null, remote_post_id = null, remote_url = null, upload_confirmed_at = null, proof_json = null, has_upload_proof = false, and error_message = Deleted YouTube Shorts test upload uQGmdxd0TeM at user request; remote proof cleared. Backups before DB edits: C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite.before-youtube-short-delete-2026-07-12T21-36-52-066Z.bak and C:\Users\chris\AppData\Roaming\nollywood-ai-pipeline\nollywood-pipeline.sqlite.before-youtube-short-delete-proofjson-clear-2026-07-12T21-46-04-314Z.bak.

Follow-up 2026-07-12 YouTube Shorts post-deletion smoke:
- Updated scripts/youtube-ui-smoke.mjs so copied-DB smoke accepts a YouTube job with no remote_post_id after the test upload was deleted, while still verifying any remote proof when present.
- Reran node scripts/youtube-ui-smoke.mjs against copied AppData; source DB job 1 remained upload_failed with remote fields null; copied smoke saw jobCountBefore = 1, jobCountAfterPrepare = 1, proofTextVisible = true, confirmCalled = true, scheduleApiCalledAfterConfirmFalse = false, and no console errors.
- Final real DB readback after smoke still showed job 1 upload_failed with scheduled_date, scheduled_time, remote_post_id, remote_url, upload_confirmed_at, and proof_json all null, with has_upload_proof = false.

Follow-up 2026-07-12 YouTube Shorts productized delete/cancel path:
- Added guarded YouTube Studio delete support for previously confirmed Shorts jobs. The productized path targets only the DB remote_post_id, requires confirmDelete=true, verifies exactly one Shorts content row by remote id before deletion, clicks the row three-dot menu -> Delete forever, checks the permanent-delete dialog, and verifies the remote id disappears from the Shorts tab before clearing local proof. Title-only deletion remains forbidden.
- Added ShortsController.deleteYouTubePublishJob plus adapter/uploader support, main/preload IPC, and a renderer Delete Remote button. The button enables only when a YouTube job has remote_post_id proof; jobs without remote proof keep the delete action disabled. Successful deletion marks the publish job status deleted and clears scheduled_date, scheduled_time, remote_post_id, remote_url, upload_confirmed_at, and proof_json. Delete failures preserve remote proof and only write an error_message.
- Added regression coverage in test-shorts-youtube-uploader-guards, test-shorts-youtube-publish-foundation, and test-shorts-youtube-renderer-ui. Updated scripts/youtube-ui-smoke.mjs to capture Delete Remote button state; copied-profile smoke after the already-deleted live proof showed deleteButtonBeforePrepare/deleteButtonAfterPrepare disabled with title No remote YouTube proof to delete, confirmCalled=true, scheduleApiCalledAfterConfirmFalse=false, and no console errors. Final real DB readback remained job 1 upload_failed with all remote proof fields null.
